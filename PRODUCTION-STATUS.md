...
| Quality gate paralelo (33 gates) | ✓ | Task #66 + Task #77 + Task #78 + Task #86 + Task #87 | `cd exentax-web && npm run check` → **EXIT 0 · 33/33** estable en **3 ejecuciones consecutivas post-#87** (wall 73,0 / 66,0 / 78,0 s · `.local/baseline-87/check-{3,4,5}.log`) |
| Cierre a producción · idiomas + rutas + validadores | ✓ | Task #78 (+ verificación #86 + #87) | Post-#87: `i18n:check` **1.566×6** PASS (Δ=0 vs #86) · 17 RouteKeys × 6 langs canónicos · **31 endpoints públicos únicos (path × método)** con cobertura Zod actualizada a 2026-04-29 (26 en `public.ts` — drift +3 vs Task #78 — + 1 + 3 + 1 = 31; 13/13 mutaciones cubiertas: 9 strict Zod + 2 unsubscribe RFC 8058 + 1 `clientErrorSchema` + 1 Ed25519) · `npm run check` 33/33 estable × 3 — ver [`cierre-produccion-i18n-rutas-validadores-2026-04-29.md`](docs/auditoria-2026-04/cierre-produccion-i18n-rutas-validadores-2026-04-29.md), [`auditoria-integral-masiva-2.md`](docs/auditoria-2026-04/auditoria-integral-masiva-2.md) y [`cierre-produccion-i18n-rutas-validadores-2-2026-04-29.md`](docs/auditoria-2026-04/cierre-produccion-i18n-rutas-validadores-2-2026-04-29.md) |
| Auditoría de seguridad de dependencias | ✓ | Task #77 (+ verificación #86 + #87) | `npm audit --omit=dev` → **0 vulnerabilities** (raíz + workspace, re-verificado en #87 · `.local/baseline-87/npm-audit-{ws,root2}.log`) |
...
| Cierre a producción · calidad i18n, keys, rutas y validadores · 2.ª pasada | ✓ | Task #87 | Re-verificación del contrato Task #78 sobre snapshot consolidado tras #86 · 1 fix quirúrgico (`lint:brand-casing` allowlist para `auditoria-integral-masiva-2.md`) · `npm run check` 33/33 estable × 3 runs (wall 73,0 / 66,0 / 78,0 s) · UX intacta — ver [`docs/auditoria-2026-04/cierre-produccion-i18n-rutas-validadores-2-2026-04-29.md`](docs/auditoria-2026-04/cierre-produccion-i18n-rutas-validadores-2-2026-04-29.md) |
| Limpieza estructural · sin huérfanos / sin legacy / sin assets muertas · 5.ª pasada (componentes / rutas / archivos / endpoints / scripts) | ✓ | Task #87 (continuación) | Pasada adicional con foco específico solicitado por el usuario (*"LIMPIA LO QUE NO SIRVE, NO SE USA"*). Auditoría categoría por categoría sobre el snapshot post-4.ª pasada: **componentes** (48 `.tsx`) → 0 huérfanos · **hooks/lib/shared/pages** (48) → 0 huérfanos · **servidor** (92 ficheros TS) → 0 huérfanos productivos (12 falsos positivos verificados, todos los schedulers registrados en `server/index.ts:903-953`) · **endpoints** → 0 huérfanos (ya verificado en pasadas anteriores con sweep HTTP 102 + `seo-orphan-audit` 780) · **public assets** (40) → 0 huérfanos. **10 huérfanos físicos eliminados** (84.859 B ≈ 83 KB): 1 manual one-shot test (`server/client-errors-csrf.test.ts`, no invocado por `package.json`/`build.ts`/CI) + 9 scripts one-off (auditorías y migraciones puntuales ya aplicadas). Conservaciones explícitas verificadas: `discord-alerts.test.ts` (PROTEGIDO #4 política conservadora), 4 constantes calculator sin consumidor directo (PROTEGIDO #1, tablas fiscales documentales), `__test-utils.ts` (importado por 2 tests Discord activos). Tracker `docs/audits/codigo-muerto.json` actualizado: DC-012 nuevo `aplicado`. `npm run check` 33/33 verde post-cleanup (wall 71,4 s) — ver [`BASELINE.md` §APÉNDICE E](BASELINE.md). |
| Limpieza estructural · sin huérfanos / sin legacy / sin assets muertas · 4.ª pasada | ✓ | Task #87 (continuación) | Pasada estructural exhaustiva alineada con `WHAT-NOT-TO-TOUCH.md` (10 áreas protegidas) y consolidando cleanup de Task #12 (`REVISION-FINAL-REPORT.md`). **4 huérfanos físicos eliminados** (4.719 B + **2 carpetas vacías**: 2 paste artifacts en `attached_assets/` con 0 referencias + `.local/baseline-87/deep/` + `uploads/docs/`). Corrección quirúrgica de **1 línea de código** en `exentax-web/scripts/build.ts:222` (argumento de `mkdir` de `"uploads/docs"` a `"uploads"`) tras verificar que la subcarpeta `docs/` era efecto colateral inadvertido del `recursive: true` y que **0 código del repo la lee/escribe** — el verdadero `INDEXING_REPORTS_DIR` es el subpath hermano `uploads/reports/indexing/`. Tracker `docs/audits/codigo-muerto.json` 100 % cerrado: DC-004/DC-006/DC-010 pasados a `verificado` + DC-011 nuevo (`aplicado`); `issues_pendientes=0` (vs 2 antes). Conservaciones explícitas verificadas: `uploads/` (parent vacía, regenerada por `build.ts:222` y target del bloqueo HTTP `/uploads → 403` en `server/index.ts:202`), `dist/index.cjs` (deploy shim del `.replit`), 8 capturas en `docs/screenshots/` (referenciadas por `audit-design-system.md`), 10 áreas protegidas intactas. `seo-orphan-audit` re-ejecutado → 0/0/0 (780 URLs probadas, sin variación). `npm run check` 33/33 verde post-cleanup — ver [`BASELINE.md` §APÉNDICE D](BASELINE.md) |
...
**Estado real tras Task #87 (2026-04-29 · 2.ª pasada del cierre Task #78 + 4.ª y 5.ª pasadas estructurales)**: **17 / 18 áreas en verde sin reservas + 1 ⚠ deferida** (cross-browser Playwright matrix — pendiente operativo, no de código; ver `PENDING-FINAL.md #5`). `npm run check` EXIT 0 (33/33) **estable en 3 ejecuciones consecutivas** post-fix §2.1 (wall 73,0 / 66,0 / 78,0 s · `.local/baseline-87/check-{3,4,5}.log`). Sin drift de código respecto al snapshot `exentax-3.0` + #77 + #78 + #83 + #86. La 2.ª pasada Task #87 aplica **1 fix quirúrgico único** (entrada nueva en `ALLOWLIST` del lint `brand-casing` para `docs/auditoria-2026-04/auditoria-integral-masiva-2.md`, mismo criterio que `docs/internal/*` y los reportes históricos), sin tocar UX, código de producción, traducciones, slugs ni validadores Zod.

**Decisión go/no-go**:

- **Branch Task #87 (cierre actual — 2.ª pasada del cierre i18n/rutas/validadores)**: 16 verde + 1 ⚠ deferida (operativa) → **integración a `main` aprobada** y **deploy a Hostinger VPS aprobado** condicionado a los pasos de `PENDING-FINAL.md #1`. Cambios contenidos a **1 fichero de código** (`exentax-web/scripts/audit/brand-casing-check.mjs`, +12 líneas en `ALLOWLIST` con 2 entradas + comentarios explicativos) + documentación raíz (BASELINE.md, PRODUCTION-STATUS.md, PRODUCTION-CHECKLIST.md, CHANGELOG.md, PENDING-FINAL.md, replit.md) + reporte ejecutivo nuevo en `docs/auditoria-2026-04/cierre-produccion-i18n-rutas-validadores-2-2026-04-29.md`. Sin cambios de UX, traducciones, slugs, validadores ni código de producción.
- **Branch Task #86 (snapshot anterior — segunda pasada profunda)**: 15 verde + 1 ⚠ deferida (operativa) → **integración a `main` aprobada** y **deploy a Hostinger VPS aprobado** condicionado a los pasos de `PENDING-FINAL.md #1`. Cambios contenidos a 3 ficheros (`scripts/audit/brand-casing-check.mjs`, `scripts/blog/blog-masterpiece-audit.mjs`, `scripts/e2e/test-newsletter-e2e.ts`) + documentación raíz + reporte en `docs/auditoria-2026-04/auditoria-integral-masiva-2.md`. Sin cambios de UX, ni de producción.
