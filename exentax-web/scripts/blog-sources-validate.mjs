 ...
 *
 * Exits 0 on zero criticals. Wired by `scripts/blog-validate-all.mjs`.
 * ----------------------------------------------------------------------------
 */
...
// dev to re-run `node scripts/blog-verify-source-urls.mjs`). 14 days is
 ...
 *  Refresh the cache by running `node scripts/blog-verify-source-urls.mjs`. */
function checkPingCacheLiveness(criticals) {
  ...
  if (!ping) {
    criticals.push({
      ...
      detail: `${rel} is missing. Run: node scripts/blog-verify-source-urls.mjs`,
    });
    return;
  }
