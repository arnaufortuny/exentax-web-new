...
/**
 * Task #44 — Exported entry point so the in-process scheduler (and any future
 * caller) can invoke the audit without spawning a child process. The CLI
 * wrapper at the bottom keeps its existing surface (argv parsing, stdout
 * output, process.exit). Returns `{ summary, exitCode, reportPath, ... }` so
 * callers can build their own surface (Discord embeds, structured logs, …)
 * without re-implementing the audit logic. `log: true` mirrors the historical
 * stdout output; `json: true` emits the machine-readable JSON dump; `write:
 * true` updates `reports/seo/lote6-numeric-hook.md`. All three default off
...
export function runAudit({
  ...
  const auditRows = [];
  // drift_real: snapshot has a footprint for (slug, lang) but the on-disk
  // first paragraph no longer matches the recorded text — i.e., we injected
  // something and then the article changed. Resolve with `--replace-existing
  // --slug`.
  const driftReal = [];
  ...
  // hook the editor changed, OR a legitimate native numeric lead.
  ...
  const slugs = Object.keys(HOOKS).sort();
    ...
    if (json) {
      ...
      console.log(`Hooks (slug x lang): ${summary.hooks}`);
      console.log(
        `Snapshot footprints loaded: ${summary.snapshotEntries} (${summary.snapshotConfirmed} match on-disk paragraph)`,
      );
      console.log(`  [OK] Injected and current (verbatim at top of article): ${summary.injectedAndCurrent}`);
      ...
      console.log(`  [XX] Drift real (snapshot proves we wrote it, paragraph or JSON moved): ${summary.driftReal}`);
      console.log(`  [??] Heuristic candidate (no snapshot, looks like a hook but != JSON — informational): ${summary.heuristicCandidate}`);
      console.log(`  [WARN] Stale year anchors (<= ${referenceYear - 2}): ${summary.staleYearAnchors}`);
    ...
    // Exit code: only `drift_real` (snapshot-confirmed editor edits or
    // yearly-bump no-ops), `needsInjection`, and the Task #43 invariant block
    // CI. `heuristic_candidate` is informational only — it captures legitimate
    // native numeric leads that the old heuristic mis-flagged, so it must NOT
    // fail the build.
    let exitCode = 0;
    if (unclassifiedStale.length > 0) exitCode = 1;
    else if (summary.needsInjection > 0 || summary.driftReal > 0) exitCode = 1;
  ...
  // Internal report writer — moved here from the old top-level `if (wantWrite)`
  // block so `runAudit({ write: true })` produces the same Markdown ledger the
  // CLI used to emit. Kept as a private function (no `export`) since callers
  // should always go through `runAudit`.
  function writeYearlyReport({
    summary,
    auditRows,
    driftReal,
    heuristicCandidate,
  ...
  // ---- CLI entrypoint ------------------------------------------------------
  // Only fires when executed directly (`node scripts/.../blog-numeric-hook-
  // yearly-refresh.mjs`); when imported (e.g. by the in-process scheduler in
  // `server/scheduled/periodic-reports.ts`), this block is skipped so the
  // importer can drive the audit programmatically via `runAudit({...})`.
  const _isMainModule =
    process.argv[1] && path.resolve(process.argv[1]) === __filename;
  if (_isMainModule) {
    const _args = new Set(process.argv.slice(2));
    const _wantWrite = _args.has("--write-report");
    const _wantJson = _args.has("--json");
    const _referenceYear = (() => {
      const idx = process.argv.indexOf("--year");
      if (idx > 0 && process.argv[idx + 1]) return parseInt(process.argv[idx + 1], 10);
      return new Date().getFullYear();
    })();
    const _result = runAudit({
