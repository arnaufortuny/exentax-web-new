 ...
 *
 * Usage:
 *   node scripts/audit/audit-bundle.mjs              # builds and audits
 *   node scripts/audit/audit-bundle.mjs --no-build   # audits the existing dist
 *   node scripts/audit/audit-bundle.mjs --json       # machine-readable JSON only
 *   node scripts/audit/audit-bundle.mjs --history    # also append to the local
 *                                              # history (.local-audit/);
 *                                              # implicit when CI=true
 *   node scripts/audit/audit-bundle.mjs --no-history # force-skip the history
...
function main() {
  ...
  if (jsonOnly) {
    ...
    if (offenders.length === 0) {
      ...
      log("scripts/audit/audit-bundle.mjs (BUDGETS_KB) y documenta el motivo en");
