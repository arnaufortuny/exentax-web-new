 ...
 *   node scripts/audit-bundle-diff.mjs --threshold=10   # custom % threshold
 *   node scripts/audit-bundle-diff.mjs --json
 *
 * Exit codes:
...
function readHistory() {
  try {
    ...
    console.error(
      "Para generar el primer registro en local ejecuta: `node scripts/audit-bundle.mjs --history`",
    );
    console.error(
...
function main() {
  ...
  if (entries.length === 0) {
    if (JSON_ONLY) {
      ...
      log(
        "Histórico vacío — ejecuta `node scripts/audit-bundle.mjs --history` " +
  ...
  if (regressions.length === 0) {
    ...
    log("presupuesto correspondiente en scripts/audit-bundle.mjs (BUDGETS_KB).");
  }
