...
import { evictExpired, enforceMaxSize } from "./store-cleanup";
...
class MemoryStore implements RateLimitStore {
  ...
  async check(limiterKey: string, entryKey: string, maxRequests: number, windowMs: number): Promise<boolean> {
    ...
    if (!entry || now > entry.resetAt) {
      ...
      enforceMaxSize(map, this.MAX_ENTRIES);
      return true;
    }
    entry.count++;
  ...
  async cleanup(): Promise<void> {
    const now = Date.now();
    const cleaned = evictExpired(this.maps.values(), (entry: RateLimitEntry) => now > entry.resetAt);
