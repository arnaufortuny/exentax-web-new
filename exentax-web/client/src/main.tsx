...
// Install global error / unhandled-rejection handlers as early as possible
// so any boot-time crash (chunk load, hydration) is reported to the backend.
clientLogger.init();
...
// Wait for the detected language bundle to load before first paint so the
// initial render is already in the correct locale. Falls back to mounting
// in Spanish if the locale chunk fails (handled inside i18nReady).
