...
function NeonCard({ title, children, className = "" }: { title?: string; children: React.ReactNode; className?: string }) {
  return (
    <div className={`rounded-2xl p-6 sm:p-7 border border-[rgba(0,229,16,0.20)] bg-[rgba(0,229,16,0.03)] ${className}`}>
      {title && <h4 className="font-heading font-bold text-base text-[var(--text-1)] mb-5">{title}</h4>}
      {children}
    </div>
  );
}
  ...
  return (
    <>
      <SEO
        ...
        ogTitle={t("comoFuncionaPage.ogTitle", { defaultValue: "" }) as string || undefined}
        ogDescription={t("comoFuncionaPage.ogDesc", { defaultValue: "" }) as string || undefined}
          ...
          <div className="text-center max-w-[800px] mx-auto">
            <div className="reveal">
              ...
              <div className="flex flex-col sm:flex-row gap-4 mb-8 justify-center">
                <Link href={lp("book")} className="inline-flex items-center justify-center btn-primary px-8 py-3.5 text-base rounded-full whitespace-nowrap sm:min-w-[220px] lg:min-w-[270px]" data-testid="button-at-hero-agendar">
                ...
                <a href={`${CONTACT.WHATSAPP_URL}?text=${encodeURIComponent(t("comoFunciona.hero.waText"))}`} target="_blank" rel="noopener noreferrer" onClick={() => trackWhatsAppClick("how_we_work_hero")} className="inline-flex items-center justify-center gap-2 btn-wa font-body font-semibold px-8 py-3.5 text-base rounded-full whitespace-nowrap sm:min-w-[220px] lg:min-w-[270px]" data-testid="button-at-hero-whatsapp">
    ...
    <section className="section-padding bg-[var(--bg-0)]" ref={ref}>
      <div className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="reveal flex flex-col sm:flex-row gap-0 sm:gap-8 lg:gap-10" data-testid="card-phase-1">
          ...
          <div className="flex-1 min-w-0 pb-12 sm:pb-20">
            <div className="mb-2 flex items-center gap-3">
              <span className="font-body font-medium text-sm text-[var(--text-3)]">{t("comoFunciona.phases.phase1.label")}</span>
            ...
            <div className="bg-[var(--bg-2)] border border-[var(--border-subtle)] rounded-2xl p-6 sm:p-8 text-center">
              ...
              <div className="flex flex-col sm:flex-row gap-4 justify-center items-center">
                <Link href={lp("book")} className="inline-flex items-center justify-center btn-primary px-8 py-3.5 text-base rounded-full whitespace-nowrap" data-testid="button-at-phase1-agendar">
                  {t("comoFunciona.phases.phase1.cta.btnAgendar")}
                </Link>
                <a href={`${CONTACT.WHATSAPP_URL}?text=${encodeURIComponent(t("comoFunciona.phases.phase1.cta.waText"))}`} target="_blank" rel="noopener noreferrer" onClick={() => trackWhatsAppClick("how_we_work_phase1")} className="inline-flex items-center justify-center gap-2 btn-wa font-body font-semibold px-8 py-3.5 text-base rounded-full whitespace-nowrap" data-testid="button-at-phase1-whatsapp">
                  <svg width={18} height={18} viewBox="0 0 24 24" fill="currentColor" className="flex-shrink-0">
                    <path d="M17.472 14.382c-.297-.149-1.758-.867-2.03-.967-.273-.099-.471-.148-.67.15-.197.297-.767.966-.94 1.164-.173.199-.347.223-.644.075-.297-.15-1.255-.463-2.39-1.475-.883-.788-1.48-1.761-1.653-2.059-.173-.297-.018-.458.13-.606.134-.133.298-.347.446-.52.149-.174.198-.298.298-.497.099-.198.05-.371-.025-.52-.075-.149-.669-1.612-.916-2.207-.242-.579-.487-.5-.669-.51-.173-.008-.371-.01-.57-.01-.198 0-.52.074-.792.372-.272.297-1.04 1.016-1.04 2.479 0 1.462 1.065 2.875 1.213 3.074.149.198 2.096 3.2 5.077 4.487.709.306 1.262.489 1.694.625.712.227 1.36.195 1.871.118.571-.085 1.758-.719 2.006-1.413.248-.694.248-1.289.173-1.413-.074-.124-.272-.198-.57-.347m-5.421 7.403h-.004a9.87 9.87 0 01-5.031-1.378l-.361-.214-3.741.982.998-3.648-.235-.374a9.86 9.86 0 01-1.51-5.26c.001-5.45 4.436-9.884 9.888-9.884 2.64 0 5.122 1.03 6.988 2.898a9.825 9.825 0 012.893 6.994c-.003 5.45-4.437 9.884-9.885 9.884m8.413-18.297A11.815 11.815 0 0012.05 0C5.495 0 .16 5.335.157 11.892c0 2.096.547 4.142 1.588 5.945L.057 24l6.305-1.654a11.882 11.882 0 005.683 1.448h.005c6.554 0 11.89-5.335 11.893-11.893a11.821 11.821 0 00-3.48-8.413z" />
                  </svg>
        ...
        <div className="reveal flex flex-col sm:flex-row gap-0 sm:gap-8 lg:gap-10" data-testid="card-phase-2">
          ...
          <div className="flex-1 min-w-0 pb-12 sm:pb-20">
            <div className="mb-2 flex items-center gap-3">
              <span className="font-body font-medium text-sm text-[var(--text-3)]">{t("comoFunciona.phases.phase2.label")}</span>
        ...
        <div className="reveal flex flex-col sm:flex-row gap-0 sm:gap-8 lg:gap-10" data-testid="card-phase-3">
          ...
          <div className="flex-1 min-w-0 pb-12 sm:pb-20">
            <div className="mb-2 flex items-center gap-3">
              <span className="font-body font-medium text-sm text-[var(--text-3)]">{t("comoFunciona.phases.phase3.label")}</span>
        ...
        <div className="reveal flex flex-col sm:flex-row gap-0 sm:gap-8 lg:gap-10" data-testid="card-phase-4">
          ...
          <div className="flex-1 min-w-0">
            <div className="mb-2 flex items-center gap-3">
              <span className="font-body font-medium text-sm text-[var(--text-3)]">{t("comoFunciona.phases.phase4.label")}</span>
          ...
          {cards.map((card, i) => (
            ...
            >
              <div className="flex flex-col h-full">
                <div className="mb-5 [&_svg_*]:!stroke-[#0B0D0C]">{card.icon}</div>

                <h3 className="text-[20px] sm:text-[22px] lg:text-[24px] leading-[1.15] mb-4 break-words" style={{ color: '#0B0D0C', hyphens: 'auto' }}>
                  {card.title}
                </h3>
                      ...
                      {(card as any).list.map((item: string) => (
                        <div key={item} className="flex items-start gap-3">
                          <svg xmlns="http://www.w3.org/2000/svg" width={20} height={20} viewBox="0 0 48 48" fill="none" className="flex-shrink-0 mt-0.5">
                            ...
                            <path d="M16.5 24.5l5.2 5.2L32 19.4" stroke="#0B0D0C" strokeWidth="3.2" strokeLinecap="round" strokeLinejoin="round"/>
                          </svg>
                          <span className="text-[15px] leading-relaxed" style={{ color: 'rgba(11,13,12,0.88)' }}>{item}</span>
                        </div>
                      ))}
                    </div>
                  )}
                ...
                {card.emphasis && (
                  <p className="mt-5 pt-4 border-t border-[rgba(11,13,12,0.18)] font-medium text-[15px] leading-relaxed" style={{ color: '#0B0D0C' }}>{card.emphasis}</p>
                )}
              </div>
            </div>
          ))}
...
export default function ComoFuncionaPage() {
  return (
    <article data-page="how-we-work" className="bg-[var(--bg-0)]">
      <ProcessHero />
      <WhyUsSection />
      <PhasesIntro />
      <PhasesTimeline />
