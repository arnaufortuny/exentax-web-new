...
- **Portugal (CIRC, CIRS, AT).** Para residentes fiscais em Portugal, a LLC americana operacional é normalmente tratada como entidade transparente: os lucros líquidos atribuem-se ao sócio na sua categoria B do IRS (Anexo B ou Anexo J quando recebidos do estrangeiro), com a tributação progressiva habitual e taxas marginais até 48 % mais a sobretaxa de solidariedade nos escalões superiores. Caso a LLC opte por tributar como corporation (Form 8832) e o sócio detenha o controlo, pode aplicar-se o regime de imputação de lucros (CFC) do art. 66.º do CIRC se a entidade estrangeira estiver sujeita a regime fiscal claramente mais favorável (taxa efetiva inferior a 60 % do IRC português, ou seja inferior a aproximadamente 12,6 %) e os rendimentos forem maioritariamente passivos. Contas bancárias norte-americanas declaram-se em Anexo J e, quando aplicável, no Modelo 58 do Banco de Portugal (Comunicação de Operações e Posições com o Exterior — COPE) sempre que o saldo agregado supere o limiar anual fixado pelo BdP. Residentes não habituais (RNH) e o novo regime de Incentivo Fiscal à Investigação Científica e Inovação (IFICI) podem alterar a base de cálculo: convém validar caso a caso com a AT.
- **Brasil (RFB, BCB).** Para residentes fiscais no Brasil, a LLC entra no perímetro da Lei 12.973/2014 (regime de tributação em bases universais para pessoas jurídicas) e, para pessoa física controladora, da Lei 14.754/2023 (tributação anual de offshores controladas a 15 %, com possibilidade de regime opcional de transparência). A declaração obrigatória inclui DCBE/CBE ao Banco Central do Brasil quando os ativos no exterior superam US$ 1 milhão (ou US$ 100 mil para a CBE trimestral), DIRF para retenções e a inclusão da LLC e respetivas contas no IRPF (Ficha de Bens e Direitos, grupo 03 — Participações Societárias). A e-Financeira reporta saldos e rendimentos via FATCA bilateral Brasil–EUA desde 2014. Trading no Interactive Brokers ou Tradovate via LLC ainda está sujeito ao GCAP mensal e à apuração específica de ganhos no exterior se classificados como rendimentos do residente brasileiro.
- **Angola (CGT, BNA).** Em Angola, residentes fiscais com participações em LLCs declaram os rendimentos atribuíveis no Imposto sobre Aplicação de Capitais (IAC) ou no Imposto sobre os Rendimentos do Trabalho (IRT) consoante a natureza, e seguem as instruções de repatriamento do Banco Nacional de Angola para movimentação de divisas; o controlo cambial impõe registo prévio das operações com o exterior através da banca comercial autorizada. A reforma tributária angolana introduziu o Código Geral Tributário (Lei 21/14) com regras antiabuso aplicáveis a estruturas estrangeiras sem substância, alinhadas com os princípios OCDE/BEPS.
