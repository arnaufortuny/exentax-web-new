export default `Se vens de recibos verdes ou de MEI, provavelmente habituaste-te a usar a conta profissional para pagar o supermercado. Com uma LLC, este reflexo destrói a proteção jurídica: um juiz americano fura o véu („pierce the corporate veil”) em três movimentos suspeitos. E as Finanças, por outro lado, podem requalificar esses fluxos como rendimentos pessoais não declarados.

Neste artigo explicamos-te por que é tão importante, que consequências tem misturar fundos, e como implementar uma separação financeira sólida desde o primeiro dia.

## Por que é tão importante separar as tuas finanças?

A LLC (Limited Liability Company) existe para separar o teu património pessoal do do teu negócio. Se a tua empresa tiver uma dívida ou um problema, os teus bens pessoais, a tua casa, as tuas poupanças, o teu carro. estão protegidos. Essa é a razão principal pela qual constituis uma LLC.

Para manter essa proteção sólida, é fundamental demonstrar que a tua LLC opera como uma entidade independente. A melhor forma de o fazer é mantendo as tuas finanças separadas de forma clara e consistente.

### As vantagens de uma boa separação financeira

Quando manténs as tuas finanças bem separadas:

- **A tua proteção patrimonial fortalece-se.** A LLC funciona como escudo real entre o teu negócio e os teus bens pessoais.
- **A tua contabilidade é mais clara.** Identificar receitas, despesas e lucros torna-se simples.
- **O cumprimento fiscal é mais fácil.** O Form 5472 e outras declarações preparam-se sem complicações.

### Boas práticas que reforçam a tua estrutura

Estes são os hábitos que os empreendedores mais organizados seguem:

- Usar uma conta bancária exclusiva para a LLC (Mercury, Relay, etc.)
- Documentar todas as transferências entre a LLC e o proprietário
- Manter registos contabilísticos separados
- Ter um Operating Agreement atualizado
- Capitalizar a LLC adequadamente desde o início
- Não misturar despesas pessoais com despesas empresariais
## Requisitos legais para a separação financeira

Manter a separação financeira não é apenas uma boa prática. é um requisito legal implícito na estrutura da LLC. Estes são os pontos que deves cumprir:

### 1. Conta bancária separada e exclusiva

A tua LLC deve ter a sua própria conta bancária, em nome da empresa, com o seu EIN como identificação fiscal. Nunca deves usar a tua conta pessoal para receber pagamentos de clientes ou pagar despesas do negócio.

### 2. Registos contabilísticos independentes

Deves manter uma contabilidade separada para a tua LLC. Isto inclui um registo de receitas, despesas, transferências ao proprietário e qualquer transação entre a empresa e tu como pessoa.

### 3. Documentação de transações entre partes relacionadas

Cada vez que transferires dinheiro entre a tua LLC e tu, seja como distribuição de lucros, reembolso de despesas ou empréstimo. deve estar documentado. Estas transações reportam-se no Form 5472 perante o <a href="https://www.irs.gov" target="_blank" rel="noopener">IRS</a>.

### 4. Operating Agreement vigente

O teu Operating Agreement deve estabelecer claramente como se distribuem os lucros, como se aprovam as despesas e quais são as regras financeiras da LLC. Sem este documento, a estrutura da tua empresa carece de formalidade.

### 5. Capitalização adequada

A tua LLC deve ter fundos suficientes para operar. Se a empresa não tem capital próprio e tu pagas tudo da tua conta pessoal, é um sinal de que não existe separação real.
## Como configurar contas bancárias separadas

A base da separação financeira é ter uma conta bancária exclusiva para a tua LLC. As duas opções mais populares para não residentes são Mercury e Relay.

### Mercury

Mercury é a plataforma fintech mais utilizada por donos de LLC não residentes. Os teus depósitos são custodiados no Column NA (banco com licença federal e seguro FDIC). As suas vantagens para manter a separação financeira:

- **Conta em nome da LLC** com o EIN como identificação
- **Sem comissões mensais** nem mínimos de saldo
- **Cartões de débito virtuais e físicos** vinculados à conta empresarial
- **Integrações contabilísticas** com QuickBooks, Xero e outras ferramentas
- **Transferências ACH e wire** para pagamentos nacionais e internacionais
- **Múltiplas sub-contas** para organizar fundos (operacional, impostos, reserva)

A abertura é 100% online e não requer viajar aos Estados Unidos. Precisas da tua LLC constituída, o EIN, o Operating Agreement e o teu passaporte válido.

### Relay

Relay é outra excelente opção, especialmente se precisas de organizar o teu dinheiro em diferentes categorias:

- **Até 20 contas correntes** sem custo adicional. podes criar uma para despesas operacionais, outra para impostos, outra para o teu salário
- **Sem comissões mensais** nem mínimos
- **Cartões de débito** atribuíveis a contas específicas
- **Integração com software contabilístico**
- **Abertura remota** para não residentes

Relay destaca-se pelo seu sistema de sub-contas, que facilita enormemente a organização financeira e a separação de fundos por categoria.

### Configuração recomendada

Uma estrutura de contas eficaz para a tua LLC poderia ser:

1. **Conta principal (receitas):** onde recebes todos os pagamentos de clientes
2. **Conta de despesas operacionais:** para pagar ferramentas, serviços e fornecedores
3. **Conta de impostos:** onde reservas uma percentagem de cada receita para obrigações fiscais
4. **Conta de distribuições:** de onde te pagas a ti mesmo de forma documentada

Esta estrutura ajuda-te a manter a disciplina financeira e facilita a contabilidade no final do ano fiscal.
## Melhores práticas para te pagares a ti mesmo a partir da tua LLC

Como proprietário de uma Single-Member LLC, não recebes um "salário" no sentido tradicional. O que fazes é tirar **distribuições de lucros** (owner's draws). Mas estas distribuições devem ser feitas corretamente para manter a separação legal.

### Regras fundamentais

**1. Estabelece um calendário regular de distribuições.** Não retires dinheiro de forma errática. Define se te vais pagar mensal, quinzenal ou semanalmente, e mantém a consistência.

**2. Documenta cada distribuição.** Cada transferência da conta da LLC para a tua conta pessoal deve ter um registo que indique: data, montante, conceito ("distribuição de lucros - mês XX") e aprovação segundo o Operating Agreement.

**3. Transfere sempre para a tua conta pessoal.** Não uses o cartão de débito da LLC para despesas pessoais. Faz uma transferência bancária para a tua conta pessoal e a partir daí paga o que precisares.

**4. Não retires mais do que a LLC pode suportar.** Deixa capital suficiente na empresa para cobrir despesas operacionais, obrigações fiscais e um fundo de reserva. Descapitalizar a LLC é um dos fatores que os tribunais consideram para perfurar o véu corporativo.

**5. Regista as distribuições na tua contabilidade.** Cada distribuição deve aparecer nos livros da LLC como uma redução do capital do proprietário, não como uma despesa do negócio.

### Exemplo prático

Supõe que a tua LLC fatura $8,000 por mês. Uma distribuição bem estruturada poderia ser:

- **$2,000** → Conta de impostos (25% de reserva)
- **Verba operacional** → Conta de despesas operacionais (ferramentas, serviços, agente registado)
- **$500** → Fundo de reserva da LLC
- **$4,000** → Distribuição ao proprietário (transferência para a tua conta pessoal)

Desta forma, a LLC mantém capital operacional, cobres as tuas obrigações fiscais e pagas-te de maneira documentada e sustentável.
## Implicações fiscais de misturar fundos

A commingling of funds. mistura de fundos pessoais e empresariais. tem consequências fiscais sérias que vão além da perda de proteção legal.

### 1. Dificuldade para justificar despesas dedutíveis

Se misturas despesas pessoais e empresariais numa mesma conta, torna-se extremamente difícil demonstrar quais são despesas legítimas do negócio. Em caso de auditoria, o IRS pode rejeitar deduções que não possas justificar com documentação clara.

### 2. Problemas com o Form 5472

O Form 5472 requer reportar todas as transações entre a LLC e o seu proprietário. Se as tuas finanças estão bem separadas, preparar este formulário é simples e direto. Com contas misturadas, o processo complica-se desnecessariamente. Nós encarregamo-nos de preparar este formulário por ti.

### 3. Reclassificação da entidade pelo IRS

Em situações excecionais, o IRS poderia questionar a estrutura de uma LLC que não mantém uma separação financeira real. Manter as tuas finanças bem organizadas desde o princípio evita qualquer complicação futura.

### 4. Complicações no teu país de residência

Como não residente nos EUA, os rendimentos da tua LLC tributam no teu país de residência fiscal. Se as tuas finanças estão misturadas, a tua declaração de rendimentos local complica-se enormemente. As autoridades fiscais do teu país podem questionar a legitimidade da estrutura se virem que não há separação real entre ti e a empresa.

### 5. Perda de benefícios de tratados fiscais

Alguns países têm tratados de dupla tributação com os Estados Unidos que oferecem benefícios fiscais. Mas estes benefícios podem perder-se se as autoridades determinarem que a LLC não opera como uma entidade independente. algo que a mistura de fundos facilita enormemente.
## Requisitos de registo e documentação

Manter registos financeiros adequados não é opcional. É um requisito do IRS e uma peça fundamental para proteger o teu véu corporativo.

### O que deves registar

- **Todas as receitas** recebidas pela LLC, com data, montante, cliente e conceito
- **Todas as despesas** pagas pela LLC, com faturas ou recibos de suporte
- **Todas as distribuições** ao proprietário, com data, montante e documentação de aprovação
- **Todos os aportes de capital** do proprietário à LLC
- **Movimentos bancários completos**: extratos mensais da conta da LLC
- **Contratos e acordos** com clientes, fornecedores e terceiros

### Quanto tempo conservar os registos

O IRS recomenda conservar os registos fiscais durante um mínimo de **3 anos** desde a data de apresentação da declaração. No entanto, em casos de omissão significativa de rendimentos, o prazo estende-se a **6 anos**. Para maior segurança, conserva os teus registos durante **7 anos**.

### Ferramentas recomendadas

- **QuickBooks Online** ou **Xero** para a contabilidade geral
- **Mercury** ou **Relay** com as suas exportações de extratos bancários
- **Google Drive** ou **Dropbox Business** para armazenar faturas e documentação
- **Notion** ou **Airtable** para um registo organizado de transações e distribuições

A chave é ter um sistema que possas manter de forma consistente. Não importa que ferramenta uses, o que importa é que a uses sempre.
## Erros comuns que os empreendedores cometem

Depois de trabalhar com centenas de empreendedores que operam com LLCs nos Estados Unidos, estes são os erros mais frequentes que vemos:

### 1. Usar a conta da LLC para despesas pessoais

Este é o erro número um. Pagar o supermercado, Netflix, a gasolina ou um jantar com o cartão da LLC destrói a separação financeira. Não importa se "depois devolves", a mistura já aconteceu.

### 2. Receber pagamentos de clientes na conta pessoal

Se um cliente te paga na tua conta pessoal em vez da conta da LLC, essa receita não está registada como receita empresarial. Isto gera problemas contabilísticos, fiscais e legais.

### 3. Não documentar as distribuições

Muitos proprietários simplesmente transferem dinheiro da LLC para a sua conta pessoal sem qualquer registo. Sem documentação, estas transferências podem ser interpretadas como commingling em vez de distribuições legítimas.

### 4. Não ter uma conta bancária dedicada desde o início

Alguns empreendedores constituem a sua LLC mas demoram semanas ou meses a abrir a conta bancária empresarial. Entretanto, operam com a sua conta pessoal "temporariamente". Esse período sem separação pode gerar problemas.

### 5. Emprestar dinheiro à LLC sem documentar

Se aportas capital pessoal à tua LLC, por exemplo, para cobrir uma despesa inicial. deves documentá-lo como um empréstimo ou um aporte de capital. Sem documentação, a linha entre os teus fundos e os da empresa dilui-se.

### 6. Não reservar fundos para impostos

Gastar todo o dinheiro que entra na LLC sem reservar para obrigações fiscais é um erro custoso. Quando chega a data de apresentar o Form 1120 e Form 5472, muitos empreendedores descobrem que não têm fundos para pagar os serviços de compliance.

### 7. Ignorar as regras durante os primeiros meses

Muitos empreendedores pensam que as regras de separação financeira só importam quando a LLC fatura muito. Falso. A separação deve existir desde o dia um. Um tribunal avaliará o teu comportamento desde a constituição da empresa.
### Checklist de separação financeira

Para te assegurares de que estás a fazer tudo corretamente, revê esta lista:

- ✓ Conta bancária em nome da LLC (Mercury ou Relay)
- ✓ Operating Agreement assinado e atualizado
- ✓ Contabilidade separada para a LLC
- ✓ Distribuições documentadas com data, montante e conceito
- ✓ Sem despesas pessoais a partir da conta da LLC
- ✓ Sem receitas empresariais na conta pessoal
- ✓ Reserva de impostos em conta dedicada
- ✓ Registos conservados por pelo menos 7 anos
- ✓ Form 5472 apresentado corretamente todos os anos
- ✓ Aportes de capital documentados

Algumas leituras adjacentes a manter à mão: <a href="/pt/blog/seu-primeiro-mes-com-uma-llc-americana-o-que-esperar-semana">Seu primeiro mês com uma LLC americana: o que esperar semana a semana</a> e <a href="/pt/blog/como-evitar-bloqueios-de-conta-no-mercury-wise-e-revolut">Como evitar bloqueios de conta no Mercury, Wise e Revolut</a>, que afinam precisamente os contornos do que este guia explica.
## Conclusão

Separar o teu dinheiro pessoal do dinheiro da tua LLC não é um detalhe administrativo. é a base sobre a qual se sustenta toda a proteção legal e fiscal que a tua LLC te oferece. Sem separação financeira, a tua LLC é apenas um nome num papel.

A boa notícia é que implementar esta separação é simples se o fizeres desde o princípio. Uma conta bancária dedicada, um sistema de contabilidade básico e disciplina nas tuas distribuições é tudo o que precisas.

<!-- exentax:calc-cta-v1 -->
> <a href="/pt/agendar">Consulta gratuita sem compromisso</a>
<!-- /exentax:calc-cta-v1 -->

Na Exentax encarregamo-nos de que a tua LLC esteja corretamente estruturada desde o dia um. incluindo a abertura da tua conta bancária, a configuração da tua contabilidade e a apresentação de todos os formulários perante o IRS. Se queres assegurar-te de que a tua separação financeira está correta, podes agendar uma assessoria gratuita onde revemos a tua situação sem compromisso.
Na Exentax instalamos desde o dia 1 a disciplina contas/cartões/faturas para clientes lusófonos. Marca a tua consultoria gratuita: verificamos o teu setup e damos-te as regras simples que não podes voltar a quebrar.
## Referências legais e regulamentares

Este artigo apoia-se em normativa em vigor à data de hoje. Citamos as fontes principais para verificação:

- **EUA.** Treas. Reg. §301.7701-3 (classificação de entidades / *check-the-box*); IRC §882 (imposto sobre rendimentos de estrangeiros conexos com US trade or business); IRC §871 (FDAP e retenções a não residentes); IRC §6038A e Treas. Reg. §1.6038A-2 (Form 5472 para *25% foreign-owned* e *foreign-owned disregarded entities*); IRC §7701(b) (residência fiscal, *substantial presence test*); 31 U.S.C. §5336 (Corporate Transparency Act, BOI Report perante a <a href="https://www.fincen.gov" target="_blank" rel="noopener">FinCEN</a>).
- **Espanha.** Lei 35/2006 (LIRPF), arts. 8, 9 (residência), 87 (atribuição de rendimentos), 91 (CFC pessoas singulares); Lei 27/2014 (LIS), art. 100 (CFC sociedades); Lei 58/2003 (LGT), arts. 15 e 16; Lei 5/2022 (regime sancionatório do Modelo 720 após TJUE C-788/19 de 27/01/2022); RD 1065/2007 (Modelos 232 e 720); Ordem HFP/887/2023 (Modelo 721 cripto).
- **Convenção Espanha–EUA.** <a href="https://www.boe.es" target="_blank" rel="noopener">BOE</a> de 22/12/1990 (CDT original); Protocolo em vigor desde 27/11/2019 (rendimento passivo, *limitation on benefits*).
- **UE / <a href="https://www.oecd.org" target="_blank" rel="noopener">OCDE</a>.** Diretiva (UE) 2011/16, alterada pela DAC6 (mecanismos transfronteiriços), DAC7 (Diretiva (UE) 2021/514, plataformas digitais) e DAC8 (Diretiva (UE) 2023/2226, criptoativos); Diretiva (UE) 2016/1164 (ATAD: CFC, *exit tax*, assimetrias híbridas); Padrão Comum de Comunicação (CRS) da OCDE.
- **Quadro internacional.** Modelo de Convenção OCDE, art. 5 (estabelecimento estável) e Comentários; Ação 5 BEPS (substância económica); Recomendação 24 do GAFI (beneficiário efetivo).

A aplicação concreta de qualquer destas normas ao seu caso depende da sua residência fiscal, da atividade da LLC e da documentação que mantenha. Este conteúdo é informativo e não substitui aconselhamento profissional personalizado.

<!-- exentax:banking-facts-v1 -->
## Factos bancários e fiscais a precisar

A informação sobre fintechs e CRS evolui; este é o estado atual:

### Notas por provedor

- **Mercury** opera com vários bancos parceiros com licença federal e cobertura **FDIC** via sweep network: principalmente **Choice Financial Group** e **Evolve Bank & Trust**, e ainda **Column N.A.** em algumas contas legadas. Mercury não é um banco; é uma plataforma fintech suportada por esses partner banks. Se a Mercury encerrar uma conta, o saldo é normalmente devolvido **por cheque em papel enviado para a morada registada do titular**, o que pode ser um problema operacional sério para não residentes; convém manter uma conta secundária (Relay, Wise Business, etc.) como contingência.
- **Wise** tem dois produtos claramente distintos: **Wise Personal** e **Wise Business**. Para uma LLC abre-se **Wise Business**, não a conta pessoal. A entidade europeia é **Wise Europe SA (Bélgica)**, que emite IBAN BE para EUR; os IBAN lituanos só persistem em contas europeias herdadas. **Wise Business titularidade de uma LLC dos EUA fica FORA do CRS** porque a titular é uma entidade dos EUA e os EUA não são jurisdição participante do CRS; o lado USD opera via Wise US Inc. (perímetro FATCA, não CRS). Apenas uma **Wise Personal aberta por um indivíduo residente fiscal numa jurisdição CRS** (p. ex. Espanha) é reportada via Wise Europe SA (Bélgica) sobre esse indivíduo.
- **Wallester** (Estónia) é uma entidade financeira europeia com licença EMI/banco emissor de cartões. As suas contas IBAN europeias **estão dentro da Norma Comum de Comunicação (CRS)** e geram, por isso, troca automática de informação para a administração fiscal do país de residência.
- **Payoneer** opera através de entidades europeias (Payoneer Europe Ltd, Irlanda) também **no âmbito do CRS** para clientes residentes em jurisdições participantes.
- **Revolut Business**: quando associado a uma **LLC norte-americana**, o esquema típico passa pela Revolut Payments USA; os IBAN europeus (lituanos, BE) **não são emitidos por defeito** a uma LLC, são emitidos a clientes europeus do banco europeu do grupo. Se lhe oferecerem um IBAN europeu, confirme a que entidade jurídica está associada e sob que regime essa entidade reporta.
- **Tributação zero**: nenhuma estrutura LLC consegue "zero impostos" se vives num país com regras CFC/transparência fiscal ou atribuição de rendimentos. O que se consegue é **não duplicar tributação** e **declarar corretamente na residência**, não eliminá-la.

<!-- exentax:legal-facts-v1 -->
## Factos legais e de procedimento

As obrigações junto da FinCEN e do IRS mudaram em recent years; eis o estado atual:

### Pontos-chave

- **BOI / Corporate Transparency Act.** Após a **interim final rule da FinCEN de março de 2025**, a obrigação do BOI Report foi **restringida a "foreign reporting companies"** (entidades constituídas fora dos EUA e registadas para operar num estado). Uma **LLC formada nos EUA por um não residente está, hoje, fora dessa obrigação**. O estado regulatório pode mudar de novo: **reverificar em FinCEN.gov no momento da submissão**. Se a tua LLC foi constituída antes de março de 2025 e já submeteste o BOI, guarda o comprovativo e monitoriza atualizações.
- **Form 5472 + 1120 pro-forma.** Para uma **Single-Member LLC detida por um não residente**, as regulamentações finais Treas. Reg. §1.6038A-1 (em vigor desde 2017) tratam a LLC como corporation para efeitos do 5472. Procedimento: **Form 1120 pro-forma** (apenas cabeçalho: nome, morada, EIN, exercício) com **Form 5472 anexado**. Envio **por correio certificado ou fax para o IRS Service Center em Ogden, Utah**, **não via MeF/e-file** padrão. Prazo: **15 de abril**; prorrogação via **Form 7004** até **15 de outubro**. **Sanção: 25.000 USD por formulário e ano, mais 25.000 USD por cada 30 dias adicionais** de não submissão após notificação do IRS.
- **Form 1120 substantivo.** Só se aplica se a LLC tiver feito check-the-box election para C-Corp (Form 8832): tributa a 21 % federal e apresenta 1120 com valores reais. A LLC disregarded padrão **não apresenta 1120 substantivo e não paga corporate tax federal**.
- **EIN e notificações.** Sem EIN não se submete 5472 nem BOI. O IRS não avisa antes de sancionar; descobre-se quando o EIN é bloqueado ou uma submissão posterior é rejeitada.<!-- exentax:execution-v2 -->
## Separar dinheiro pessoal e da LLC: porque importa mais do que pensa

Misturar dinheiro pessoal e da LLC é o erro operativo mais comum e, paradoxalmente, o mais caro a longo prazo.

- **Perde a responsabilidade limitada (piercing the corporate veil).** Se um credor prova commingling, um juiz US pode perfurar o véu e cobrar do património pessoal.
- **Bookkeeping e compliance tornam-se inviáveis.** Multiplica horas de contabilista, eleva risco de erro no 5472.
- **O banco fecha mais cedo ou mais tarde.** Mercury, Wise, Relay têm políticas explícitas: conta business para actividade business.
- **A sua rastreabilidade fiscal em residência complica-se.** Sem separação tributa pior por incerteza.

### Como separar correctamente

Conta business exclusiva. Conta pessoal separada com transferências identificadas como "owner draw". Cartão LLC SÓ para business. Cartão pessoal para tudo pessoal.

### O que mais nos perguntam

**Se me transfiro um salário regular?** Para single-member disregarded não é "salário" mas "owner's draw".

**Posso pagar coworking com cartão LLC?** Sim, dedutível. Mas não também ginásio, café e compras de fim-de-semana.

Na Exentax ajudamos a montar o fluxo correcto desde dia 1 e reordenamos históricos misturados.
<!-- /exentax:execution-v2 -->

## Factos bancários e fiscais que convém precisar

Leia esta secção como uma checklist com mordida: cada ponto sinaliza um modo de falha real que vimos em processos LLC transfronteiriços. Não salte nenhum - a maioria das reavaliações e encerramentos de conta que limpamos remonta a um destes itens.

### Leituras adicionais relacionadas

Detalhe prático a fixar antes de agir. A maioria do dano evitável que vemos neste ponto vem de saltar a documentação, não da lógica fiscal subjacente.
### Boas práticas que reforçam a sua estrutura

Nota de campo de quem corre isto mês a mês com clientes: a regra é simples, é na execução que rebenta. Planeie o operacional antes do jurídico.

### Slash: rendimento sobre capital ocioso

### Wallester: cartões corporativos com controlo granular

Apontamento concreto dos nossos casos: é assim que acontece de facto, não como uma página comercial descreve. Os números e o calendário pesam - falhar num deles desfaz o resto.

<!-- exentax:cta-v1 -->
<!-- exentax:cta-conv-v1 -->
<p data-testid="cta-action-row">Queres falar agora? Liga-nos para <a href="tel:+34614916910">+34 614 916 910</a> ou escreve-nos por <a href="https://wa.me/34614916910?text=Ol%C3%A1%20Exentax%2C%20estou%20a%20ler%20%22Se%20vens%20de%20recibos%20verdes%20ou%20de%20MEI%2C%20provavelmente%20habituaste-te%20a%20usar%20a%20con%E2%80%A6%22%20e%20quero%20falar%20com%20um%20consultor%20sobre%20o%20meu%20caso.">WhatsApp</a> e respondemos hoje.</p>

Se preferes falar diretamente, <a href="/pt/agendar">marca uma sessão gratuita</a> e analisamos o teu caso real em trinta minutos.
<!-- /exentax:cta-conv-v1 -->

Marque uma consulta gratuita de 30 minutos: analisamos o seu caso real e dizemos-lhe o que faz sentido. <a href="/pt/agendar">Marcar consulta gratuita</a>.
<!-- /exentax:cta-v1 -->

<!-- exentax:review-anchor-v1 -->
<aside data-testid="review-anchor" class="text-xs text-muted-foreground border-t pt-4 mt-8">
<p><strong>Revisão editorial pendente</strong> — As referências seguintes requerem verificação manual contra a fonte oficial vigente. Se identificares uma divergência, escreve à equipa e corrigimos em menos de 24 horas.</p>
<ul class="list-disc pl-5 space-y-1">
<li><span class="font-mono">100%</span> <span class="opacity-70">(valor)</span> <span class="text-xs italic">— «…izar fondos (operativa, impuestos, reserva) La apertura es 100% online y no requiere viaja…»</span> <strong>[NÃO VERIFICADO]</strong></li>
<li><span class="font-mono">5,000</span> <span class="opacity-70">(valor)</span> <span class="text-xs italic">— «…tras mantienes acceso al capital. Para una LLC que factura $5,000-10,000/mes y distribuye …»</span> <strong>[NÃO VERIFICADO]</strong></li>
<li><span class="font-mono">10,000</span> <span class="opacity-70">(valor)</span> <span class="text-xs italic">— «…antienes acceso al capital. Para una LLC que factura $5,000-10,000/mes y distribuye trimes…»</span> <strong>[NÃO VERIFICADO]</strong></li>
<li><span class="font-mono">8,000</span> <span class="opacity-70">(valor)</span> <span class="text-xs italic">— «…ocio. ### Ejemplo práctico Supongamos que tu LLC factura $8,000 al mes. Una distribución b…»</span> <strong>[NÃO VERIFICADO]</strong></li>
<li><span class="font-mono">2,000</span> <span class="opacity-70">(valor)</span> <span class="text-xs italic">— «…l mes. Una distribución bien estructurada podría ser: - **$2,000** → Cuenta de impuestos (…»</span> <strong>[NÃO VERIFICADO]</strong></li>
<li><span class="font-mono">25%</span> <span class="opacity-70">(valor)</span> <span class="text-xs italic">— «…tructurada podría ser: - **$2,000** → Cuenta de impuestos (25% de reserva) - **Asignación …»</span> <strong>[NÃO VERIFICADO]</strong></li>
<li><span class="font-mono">4,000</span> <span class="opacity-70">(valor)</span> <span class="text-xs italic">— «…te registado) - **$500** → Fondo de reserva de la LLC - **$4,000** → Distribución al prop…»</span> <strong>[NÃO VERIFICADO]</strong></li>
<li><span class="font-mono">301.770</span> <span class="opacity-70">(valor)</span> <span class="text-xs italic">— «…es para que puedas verificarlo: - **EE. UU.** Treas. Reg. §301.7701-3 (clasificación de en…»</span> <strong>[REVISIÓN MANUAL — fonte sugerida: <a href="https://www.irs.gov" rel="nofollow noopener" target="_blank">www.irs.gov</a>]</strong></li>
<li><span class="font-mono">1.603</span> <span class="opacity-70">(valor)</span> <span class="text-xs italic">— «…P y retenciones a no residentes); IRC §6038A y Treas. Reg. §1.6038A-2 (Form 5472 para *25%…»</span> <strong>[REVISIÓN MANUAL — fonte sugerida: <a href="https://www.irs.gov" rel="nofollow noopener" target="_blank">www.irs.gov</a>]</strong></li>
<li><span class="font-mono">25.000</span> <span class="opacity-70">(valor)</span> <span class="text-xs italic">— «…ga con **Form 7004** hasta el **15 de octubre**. **Sanción: 25.000 USD por formulario y añ…»</span> <strong>[NÃO VERIFICADO]</strong></li>
<li><span class="font-mono">21 %</span> <span class="opacity-70">(valor)</span> <span class="text-xs italic">— «…the-box election* a C-Corp (Form 8832): entonces tributa al 21 % federal y presenta un 112…»</span> <strong>[REVISIÓN MANUAL — fonte sugerida: <a href="https://www.irs.gov" rel="nofollow noopener" target="_blank">www.irs.gov</a>]</strong></li>
<li><span class="font-mono">IRC §882</span> <span class="opacity-70">(referência legal)</span> <span class="text-xs italic">— «…§301.7701-3 (clasificación de entidades / *check-the-box*); IRC §882 (impuesto sobre renta…»</span> <strong>[REVISIÓN MANUAL — fonte sugerida: <a href="https://www.irs.gov" rel="nofollow noopener" target="_blank">www.irs.gov</a>]</strong></li>
<li><span class="font-mono">IRC §871</span> <span class="opacity-70">(referência legal)</span> <span class="text-xs italic">— «…rentas de extranjeros conectadas con US trade or business); IRC §871 (FDAP y retenciones a…»</span> <strong>[REVISIÓN MANUAL — fonte sugerida: <a href="https://www.irs.gov" rel="nofollow noopener" target="_blank">www.irs.gov</a>]</strong></li>
<li><span class="font-mono">IRC §6038</span> <span class="opacity-70">(referência legal)</span> <span class="text-xs italic">— «…r business); IRC §871 (FDAP y retenciones a no residentes); IRC §6038A y Treas. Reg. §1.60…»</span> <strong>[REVISIÓN MANUAL — fonte sugerida: <a href="https://www.irs.gov" rel="nofollow noopener" target="_blank">www.irs.gov</a>]</strong></li>
<li><span class="font-mono">IRC §7701</span> <span class="opacity-70">(referência legal)</span> <span class="text-xs italic">— «…25% foreign-owned* y *foreign-owned disregarded entities*); IRC §7701(b) (residencia fisca…»</span> <strong>[REVISIÓN MANUAL — fonte sugerida: <a href="https://www.irs.gov" rel="nofollow noopener" target="_blank">www.irs.gov</a>]</strong></li>
<li><span class="font-mono">Form 5472</span> <span class="opacity-70">(referência legal)</span> <span class="text-xs italic">— «….** El &lt;a href=&quot;/es/blog/form-5472-que-es-como-presentarlo&quot;&gt;Form 5472&lt;/a&gt; y otras declarac…»</span> <strong>[REVISIÓN MANUAL — fonte sugerida: <a href="https://www.irs.gov" rel="nofollow noopener" target="_blank">www.irs.gov</a>]</strong></li>
<li><span class="font-mono">Form 1120</span> <span class="opacity-70">(referência legal)</span> <span class="text-xs italic">— «…es un error costoso. Cuando llega la fecha de presentar el Form 1120 y Form 5472, muchos e…»</span> <strong>[REVISIÓN MANUAL — fonte sugerida: <a href="https://www.irs.gov" rel="nofollow noopener" target="_blank">www.irs.gov</a>]</strong></li>
<li><span class="font-mono">Form 7004</span> <span class="opacity-70">(referência legal)</span> <span class="text-xs italic">— «…le** estándar. Vencimiento: **15 de abril**; prórroga con **Form 7004** hasta el **15 de o…»</span> <strong>[REVISIÓN MANUAL — fonte sugerida: <a href="https://www.irs.gov" rel="nofollow noopener" target="_blank">www.irs.gov</a>]</strong></li>
<li><span class="font-mono">Form 8832</span> <span class="opacity-70">(referência legal)</span> <span class="text-xs italic">— «…a si la LLC ha realizado *check-the-box election* a C-Corp (Form 8832): entonces tributa a…»</span> <strong>[REVISIÓN MANUAL — fonte sugerida: <a href="https://www.irs.gov" rel="nofollow noopener" target="_blank">www.irs.gov</a>]</strong></li>
<li><span class="font-mono">RD 1065/2007</span> <span class="opacity-70">(referência legal)</span> <span class="text-xs italic">— «…cionador del Modelo 720 tras STJUE C-788/19 de 27/01/2022); RD 1065/2007 (Modelos 232 y 72…»</span> <strong>[REVISIÓN MANUAL — fonte sugerida: <a href="https://www.boe.es" rel="nofollow noopener" target="_blank">www.boe.es</a>]</strong></li>
<li><span class="font-mono">DAC6</span> <span class="opacity-70">(referência legal)</span> <span class="text-xs italic">— «…oopener&quot;&gt;OCDE&lt;/a&gt;.** Directiva (UE) 2011/16, modificada por DAC6 (mecanismos transfronteri…»</span> <strong>[REVISIÓN MANUAL — fonte sugerida: <a href="https://eur-lex.europa.eu" rel="nofollow noopener" target="_blank">eur-lex.europa.eu</a>]</strong></li>
<li><span class="font-mono">DAC7</span> <span class="opacity-70">(referência legal)</span> <span class="text-xs italic">— «…2011/16, modificada por DAC6 (mecanismos transfronterizos), DAC7 (Directive (EU) 2021/514,…»</span> <strong>[REVISIÓN MANUAL — fonte sugerida: <a href="https://eur-lex.europa.eu" rel="nofollow noopener" target="_blank">eur-lex.europa.eu</a>]</strong></li>
<li><span class="font-mono">DAC8</span> <span class="opacity-70">(referência legal)</span> <span class="text-xs italic">— «…s), DAC7 (Directive (EU) 2021/514, plataformas digitales) y DAC8 (criptoactivos); Directiv…»</span> <strong>[REVISIÓN MANUAL — fonte sugerida: <a href="https://eur-lex.europa.eu" rel="nofollow noopener" target="_blank">eur-lex.europa.eu</a>]</strong></li>
</ul>
</aside>
<!-- /exentax:review-anchor-v1 -->
`;
