export default `Für einen DACH-Unternehmer ist die <a href="https://www.irs.gov" target="_blank" rel="noopener">IRS</a> das US-Pendant zum Finanzamt, funktioniert aber deutlich anders: Benachrichtigungen per Briefpost aus Austin, Texas, eigene Fallnummern pro Transaktion, und eine echte Möglichkeit, einen Agent telefonisch zu erreichen (was unsere Kunden regelmäßig überrascht). Zu verstehen, wie die IRS tickt, ist halbe Miete beim Betrieb einer LLC.

## Was ist der IRS?

Der **IRS** (Internal Revenue Service) ist die Bundessteuerbehörde der Vereinigten Staaten. Er ist das Äquivalent zu:

- **Bundeszentralamt für Steuern / Finanzamt** in Deutschland
- **BMF** in Österreich
- **ESTV** in der Schweiz
- **SAT** in Mexiko
- **AEAT** (<a href="https://sede.agenciatributaria.gob.es" target="_blank" rel="noopener">Agencia Tributaria</a>) in Spanien

Seine Hauptaufgabe ist die Erhebung von Bundessteuern und die Durchsetzung der US-Steuergesetzgebung.
## Welche Beziehung hat der IRS zu Ihrer LLC?

Auch wenn Ihre LLC keine Bundessteuern in den USA zahlt (als Disregarded Entity mit nicht-ansässigem Eigentümer), ist der IRS weiterhin relevant für Sie, denn:

### 1. Er vergibt Ihnen die EIN
Die **EIN** (Employer Identification Number) ist die Steuernummer Ihrer LLC. Sie wird vom IRS ausgestellt und ist für praktisch alles notwendig: Bankkonten eröffnen, sich bei Stripe registrieren, Steuererklärungen abgeben.

### 2. Sie reichen jährliche Erklärungen ein
Jedes Jahr müssen Sie das **Form 5472 + Form 1120** beim IRS einreichen. Es ist ein Informationsformular, das die Transaktionen zwischen Ihnen und Ihrer LLC meldet. Sie zahlen keine Steuern, aber der IRS möchte wissen, welche Bewegungen es gibt.

### 3. Er erfordert pünktliche Einhaltung
Der IRS erwartet, dass Sie das Form 5472 jedes Jahr rechtzeitig einreichen. Es ist ein wichtiger administrativer Vorgang, den wir im Rahmen unseres Unterhaltsservices für Sie erledigen.

### 4. Er bestimmt die steuerliche Klassifizierung Ihrer LLC
Der IRS ist es, der festlegt, wie Ihre LLC steuerlich behandelt wird. Für eine Single-Member LLC mit ausländischem Eigentümer lautet die Klassifizierung „Disregarded Entity", was bedeutet, dass die LLC selbst kein Steuerpflichtiger ist.
## Wie funktioniert das Steuersystem des IRS?

Das US-Steuersystem ist komplex, aber für einen nicht-ansässigen LLC-Eigentümer müssen Sie Folgendes wissen:

### Steuern für Nicht-Ansässige mit LLC
- **Einkünfte aus US-amerikanischer Quelle:** Könnten steuerpflichtig sein (FDAP income, ECI income), das trifft aber auf die Mehrheit der digitalen Freiberufler, die mit Kunden von außerhalb der USA arbeiten, nicht zu.
- **Einkünfte aus ausländischer Quelle:** Unterliegen nicht der US-Bundessteuer. Das ist der Schlüssel zum Steuervorteil der LLC für Nicht-Ansässige.

### Das Konzept der „Quelle" des Einkommens
Was bestimmt, ob Sie in den USA Steuern zahlen, ist nicht, wo Ihre LLC registriert ist, sondern **woher das Einkommen stammt**:

- Sie entwerfen eine Website für einen Kunden in Deutschland von Ihrem Zuhause in Österreich aus → Einkommen aus **ausländischer** Quelle → nicht steuerpflichtig in den USA
- Sie entwerfen eine Website für einen Kunden in New York, während Sie sich physisch in New York befinden → Einkommen aus **amerikanischer** Quelle → könnte steuerpflichtig sein

Für die Mehrheit der Freiberufler und digitalen Unternehmer stammen die Einkünfte aus ausländischer Quelle, was zu **0% Bundessteuer** führt.
### IRS-Fristen, die Sie kennen müssen

| Datum | Pflicht |
|---|---|
| 15. Januar | Beginn des Einreichungszeitraums |
| 15. April | Frist für Form 5472 + 1120 (oder Fristverlängerung beantragen) |
| 15. April | Frist für FBAR (automatische Verlängerung bis 15. Oktober) |
| 15. Oktober | Frist mit Verlängerung für Form 5472 + 1120 |
### Kann ich mit dem IRS kommunizieren?

Ja, aber es ist nicht das Schnellste auf der Welt. Der IRS hat:
- **Telefonhotline:** In der Regel mit langen Wartezeiten
- **Postkorrespondenz:** Alles Formale wird per Post erledigt
- **Online-Portal (IRS.gov):** Für Anfragen, Formulare und Tools

Für Nicht-Ansässige kann die Kommunikation mit dem IRS besonders kompliziert sein wegen der Zeitzonen und Sprachanforderungen (alles ist auf Englisch).
## Die Formulare, die Sie kennen müssen (und die wir für Sie erstellen)

### Form 5472, das Wichtigste
Es ist das Informationsformular, das die Transaktionen zwischen der LLC und ihrem ausländischen Eigentümer meldet. Es umfasst:
- Kapitaleinlagen des Mitglieds
- Ausschüttungen (Owner's Draws) an das Mitglied
- Darlehen zwischen der LLC und dem Mitglied
- Zahlungen für Dienstleistungen zwischen verbundenen Parteien

**Eine verspätete Einreichung zieht erhebliche Strafen nach sich.** Der IRS nimmt dieses Formular sehr ernst. Deshalb nehmen wir bei Exentax die Fristen sehr ernst.

### Form 1120, das „Deckblatt" des 5472
Das Form 5472 kann nicht allein eingereicht werden. Es wird zusammen mit einem Form 1120 (Körperschaftsteuererklärung) eingereicht, das im Fall einer Disregarded Entity mit Nullwerten ausgefüllt wird (da keine Steuern zu zahlen sind). Es ist im Grunde ein Umschlag für das 5472.

### Form 7004, die automatische Fristverlängerung
Wenn wir aus irgendeinem Grund zum 15. April nicht bereit sind, reichen wir ein Form 7004 ein, das eine automatische Verlängerung von 6 Monaten gewährt (bis zum 15. Oktober). Bei Exentax beantragen wir die Verlängerung proaktiv für alle unsere Kunden, sie ist kostenlos und nimmt den Stress.

### FBAR (<a href="https://www.fincen.gov" target="_blank" rel="noopener">FinCEN</a> Form 114)
Wenn die LLC Finanzkonten außerhalb der USA hat, deren aggregierter Saldo zu irgendeinem Zeitpunkt des Jahres $10.000 übersteigt, muss der FBAR eingereicht werden. Er wird elektronisch über FinCEN eingereicht, nicht über den IRS.
### Muss ich Angst vor dem IRS haben?

Nein. Ganz entschieden nein. Der IRS ist kein Monster, er ist eine Bürokratie. Eine große, langsame und manchmal nervtötende Bürokratie, aber vorhersehbar. Wenn Sie sich an die Regeln halten, haben Sie nichts zu befürchten.

Der IRS verfolgt diejenigen, die:
- Pflichtformulare nicht einreichen
- Aktiv Steuern hinterziehen
- In ihren Erklärungen lügen
- Konten oder Einkünfte verbergen

Wenn Ihre LLC ordnungsgemäß gegründet ist, Sie Ihre Formulare rechtzeitig einreichen und Ihre Tätigkeit legitim ist, weiß der IRS nicht einmal, dass Sie existieren. Sie sind eine Nummer unter Millionen von Erklärungen, die jährlich verarbeitet werden.

Wer hier mehr Tiefe vermisst, findet in <a href="/de/blog/us-llc-fur-digitale-marketingagenturen-struktur-abrechnung">US LLC für digitale Marketingagenturen: Struktur, Abrechnung und Skalierung</a> und <a href="/de/blog/us-llc-fur-software-entwickler-und-saas-grunder">US LLC für Software-Entwickler und SaaS-Gründer: vollständiger Leitfaden</a> genau die Nachbaraspekte, die wir normalerweise in eigene Texte auslagern.
### Bei Exentax rühren Sie nichts beim IRS an

Die gesamte Beziehung mit dem IRS übernehmen wir:

- Wir erstellen und reichen das Form 5472 + Form 1120 jedes Jahr ein
- Wir beantragen die Fristverlängerung (Form 7004) proaktiv
- Wir reichen den FBAR ein, falls zutreffend
- Wir beantworten jegliche Korrespondenz des IRS
- Wir halten Sie über Fristen und regulatorische Änderungen informiert
- Wir bewahren Kopien aller eingereichten Dokumente auf

Sie rufen nicht beim IRS an, Sie füllen keine Formulare aus, Sie machen sich keine Sorgen um Fristen. Das ist buchstäblich unsere Arbeit. Und wir machen sie gut.

Vereinbaren Sie eine kostenlose Beratung und wir erklären Ihnen genau, wie Ihre Beziehung zum IRS als LLC-Eigentümer funktioniert. Spoiler: Es ist viel einfacher, als es scheint.

<!-- exentax:calc-cta-v1 -->
> <a href="/de/buchen">Sprechen Sie mit unserem Team</a>
<!-- /exentax:calc-cta-v1 -->

Bei Exentax sprechen wir täglich mit der IRS für DACH-Kunden. Buchen Sie Ihre kostenlose Beratung: wir sagen Ihnen, welche Formulare Sie betreffen, welche Sie ignorieren können und wann tatsächlich Eile geboten ist (selten).
## Rechtliche und regulatorische Quellen

Dieser Artikel stützt sich auf Vorschriften, die zum Stichtag aktuell in Kraft sind. Hauptquellen zur Verifikation:

- **USA.** Treas. Reg. §301.7701-3 (Entity Classification / *check-the-box*); IRC §882 (Steuer auf mit US-Geschäft effektiv verbundene Einkünfte Ausländer); IRC §871 (FDAP und Quellensteuer bei Nicht-Residenten); IRC §6038A und Treas. Reg. §1.6038A-2 (Form 5472 für *25% foreign-owned* und *foreign-owned disregarded entities*); IRC §7701(b) (Steuerwohnsitz, *substantial presence test*); 31 U.S.C. §5336 (Corporate Transparency Act, BOI Report bei FinCEN).
- **Spanien.** Gesetz 35/2006 (LIRPF), Artt. 8, 9 (Wohnsitz), 87 (Einkünftezurechnung), 91 (Hinzurechnungsbesteuerung natürliche Personen); Gesetz 27/2014 (LIS), Art. 100 (Hinzurechnungsbesteuerung Gesellschaften); Gesetz 58/2003 (LGT), Artt. 15 und 16; Gesetz 5/2022 (Sanktionsregime Modelo 720 nach EuGH C-788/19 vom 27.01.2022); RD 1065/2007 (Modelos 232 und 720); Anordnung HFP/887/2023 (Modelo 721 Krypto).
- **DBA Spanien–USA.** <a href="https://www.boe.es" target="_blank" rel="noopener">BOE</a> vom 22.12.1990 (DBA); Protokoll in Kraft seit 27.11.2019 (passive Einkünfte, *limitation on benefits*).
- **EU / <a href="https://www.oecd.org" target="_blank" rel="noopener">OECD</a>.** Richtlinie (EU) 2011/16, geändert durch DAC6 (grenzüberschreitende Gestaltungen), DAC7 (Richtlinie (EU) 2021/514, digitale Plattformen) und DAC8 (Richtlinie (EU) 2023/2226, Kryptowerte); Richtlinie (EU) 2016/1164 (ATAD: CFC, Exit Tax, hybride Gestaltungen); OECD Common Reporting Standard (CRS).
- **Internationaler Rahmen.** OECD-Musterabkommen, Art. 5 (Betriebsstätte) und Kommentare; BEPS-Aktion 5 (wirtschaftliche Substanz); FATF-Empfehlung 24 (wirtschaftlicher Eigentümer).
Die konkrete Anwendung dieser Regeln auf deinen Fall hängt von deinem Steuerwohnsitz, der Tätigkeit der LLC und der von dir geführten Dokumentation ab. Dieser Inhalt ist informativ und ersetzt keine personalisierte professionelle Beratung.

<!-- exentax:legal-facts-v1 -->
## Rechts- und Verfahrensfakten

Die Meldepflichten gegenüber FinCEN und IRS haben sich recent years bewegt; aktueller Stand:

### Kernpunkte

- **BOI / Corporate Transparency Act.** Nach der **FinCEN Interim Final Rule vom März 2025** wurde die BOI-Meldepflicht **auf „foreign reporting companies" beschränkt** (außerhalb der USA gegründete Einheiten, die in einem Bundesstaat zur Geschäftstätigkeit registriert sind). Eine **in den USA von einem Nicht-Residenten gegründete LLC fällt heute außerhalb dieser Pflicht**. Der Regelstatus kann erneut wechseln: **Bei Einreichung auf FinCEN.gov prüfen**. Wenn Ihre LLC vor März 2025 gegründet wurde und das BOI bereits eingereicht ist, Bestätigung aufbewahren und Updates beobachten.
- **Form 5472 + Pro-forma-1120.** Für eine **Single-Member LLC im Eigentum eines Nicht-Residenten** behandeln die Schlussregelungen Treas. Reg. §1.6038A-1 (seit 2017 in Kraft) die LLC für 5472-Zwecke als Corporation. Verfahren: **Pro-forma Form 1120** (nur Kopf: Name, Adresse, EIN, Steuerjahr) mit **Form 5472 als Anlage**. Einreichung **per Einschreiben oder Fax an das IRS Service Center Ogden, Utah**, **keine E-Einreichung über das Standard-MeF**. Frist: **15. April**; Verlängerung über **Form 7004** bis **15. Oktober**. **Sanktion: 25.000 USD pro Formular und Jahr, plus 25.000 USD je weitere 30 Tage** Nichteinreichung nach IRS-Mitteilung.
- **Substantielles Form 1120.** Nur wenn die LLC per Check-the-Box-Wahl zur C-Corp optiert hat (Form 8832): dann 21 % Bundessteuer und ein substantielles 1120. Eine Standard-disregarded LLC **reicht kein substantielles 1120 ein und zahlt keine bundesstaatliche Körperschaftsteuer**.
- **EIN und Benachrichtigung.** Ohne EIN ist weder 5472 noch BOI einreichbar. Der IRS warnt nicht vor Sanktionen; man bemerkt es, wenn die EIN gesperrt oder eine spätere Einreichung abgelehnt wird.<!-- exentax:execution-v2 -->
## Was die IRS ist und praktischer Leitfaden für LLC-Eigentümer

Die IRS ist die US-Bundessteuerbehörde. Für nicht-ansässige LLC-Eigentümer nicht bürokratischer als jedes europäische Finanzamt - aber prozedural und vorhersehbar.

- **Wie die IRS Sie identifiziert.** Als Individuum per ITIN oder Abwesenheit. Als Entität per EIN Ihrer LLC (Form SS-4). EIN ist lebenslang; Adresse ändert man mit Form 8822-B.
- **Was sie von nicht-ansässiger single-member LLC verlangt.** Jährlich Form 5472 + Form 1120 pro forma (15. April, Verlängerung 15. Oktober mit Form 7004). Das 5472 meldet „reportable transactions".
- **Wann sie W-8BEN-E verlangt.** Nicht die IRS direkt, sondern jeder US-Payer (Stripe, Amazon), um keine 30% einzubehalten. Gilt 3 Jahre.
- **Wie die IRS kommuniziert.** Per Post an LLC-Adresse. NIE per Email, unaufgefordertem Anruf oder WhatsApp. Jede digitale „IRS"-Kommunikation ist Phishing. Echte Briefe haben CP-Nummer auf irs.gov verifizierbar.

### Was am häufigsten gefragt wird

**Muss ich ITIN haben?** Nein, außer bei spezifischen Bedürfnissen. Für disregarded single-member LLC reicht EIN.

**Wenn die IRS einen unverständlichen Brief schickt?** Nicht heiß antworten. CP-Code identifizieren, Modell auf irs.gov suchen, Frist (typisch 30 Tage) berechnen, EA/CPA konsultieren.

Bei Exentax sind wir Ihr IRS-Ansprechpartner.
<!-- /exentax:execution-v2 -->

## Wir richten es ein, ohne dass Sie ein Wochenende verlieren

Tausende von Freelancern und Unternehmern betreiben ihre US-LLC bereits vollständig legal und dokumentiert. Bei Exentax kümmern wir uns um den gesamten Prozess: Gründung, Banking, Zahlungsabwicklung, Buchhaltung, IRS-Erklärungen und Compliance in Ihrem Wohnsitzland. Buchen Sie eine kostenlose Beratung, und wir sagen Ihnen ehrlich, ob die LLC für Ihren Fall sinnvoll ist, ohne absolute Versprechen.

<!-- exentax:cta-conv-v1 -->
<p data-testid="cta-action-row">Möchten Sie es jetzt besprechen? Rufen Sie uns unter <a href="tel:+34614916910">+34 614 916 910</a> an oder schreiben Sie uns auf <a href="https://wa.me/34614916910?text=Hallo%20Exentax%2C%20ich%20lese%20den%20Artikel%20%22F%C3%BCr%20einen%20DACH-Unternehmer%20ist%20die%20a%20href%3Dhttps%3A%2F%2Fwww%22%20und%20m%C3%B6chte%20mit%20einem%20Berater%20%C3%BCber%20meinen%20Fall%20sprechen.">WhatsApp</a>, wir antworten heute.</p>

Wenn Sie den gesamten Prozess im Detail sehen möchten, sehen Sie sich unsere <a href="/de/leistungen">Leistungsseite</a> mit allen enthaltenen Leistungen.
<!-- /exentax:cta-conv-v1 -->

<!-- exentax:cta-v1 -->
Wir prüfen BOI, EIN, Registered Agent und Bundespflichten, damit Sie keine Strafe überrascht. <a href="/de/leistungen">Compliance-Check anfragen</a>.
<!-- /exentax:cta-v1 -->

<!-- exentax:review-anchor-v1 -->
<aside data-testid="review-anchor" class="text-xs text-muted-foreground border-t pt-4 mt-8">
<p><strong>Redaktionelle Überprüfung ausstehend</strong> — Die folgenden Verweise erfordern eine manuelle Prüfung anhand der offiziellen aktuellen Quelle. Wenn Sie eine Abweichung feststellen, schreiben Sie der Redaktion — wir korrigieren innerhalb von 24 Stunden.</p>
<ul class="list-disc pl-5 space-y-1">
<li><span class="font-mono">0%</span> <span class="opacity-70">(Kennzahl)</span> <span class="text-xs italic">— «…los ingresos son de fuente extranjera, lo que resulta en **0% de impuesto federal**. ### P…»</span> <strong>[REVISIÓN MANUAL — vorgeschlagene Quelle: <a href="https://www.irs.gov" rel="nofollow noopener" target="_blank">www.irs.gov</a>]</strong></li>
<li><span class="font-mono">10,000</span> <span class="opacity-70">(Kennzahl)</span> <span class="text-xs italic">— «…nancieras fuera de EE.UU. con un saldo agregado superior a $10,000 en cualquier momento de…»</span> <strong>[REVISIÓN MANUAL — vorgeschlagene Quelle: <a href="https://www.irs.gov" rel="nofollow noopener" target="_blank">www.irs.gov</a>]</strong></li>
<li><span class="font-mono">301.770</span> <span class="opacity-70">(Kennzahl)</span> <span class="text-xs italic">— «…es para que puedas verificarlo: - **EE. UU.** Treas. Reg. §301.7701-3 (clasificación de en…»</span> <strong>[REVISIÓN MANUAL — vorgeschlagene Quelle: <a href="https://www.irs.gov" rel="nofollow noopener" target="_blank">www.irs.gov</a>]</strong></li>
<li><span class="font-mono">1.603</span> <span class="opacity-70">(Kennzahl)</span> <span class="text-xs italic">— «…P y retenciones a no residentes); IRC §6038A y Treas. Reg. §1.6038A-2 (Form 5472 para *25%…»</span> <strong>[REVISIÓN MANUAL — vorgeschlagene Quelle: <a href="https://www.irs.gov" rel="nofollow noopener" target="_blank">www.irs.gov</a>]</strong></li>
<li><span class="font-mono">25%</span> <span class="opacity-70">(Kennzahl)</span> <span class="text-xs italic">— «…ntes); IRC §6038A y Treas. Reg. §1.6038A-2 (Form 5472 para *25% foreign-owned* y *foreign-…»</span> <strong>[REVISIÓN MANUAL — vorgeschlagene Quelle: <a href="https://www.irs.gov" rel="nofollow noopener" target="_blank">www.irs.gov</a>]</strong></li>
<li><span class="font-mono">25.000</span> <span class="opacity-70">(Kennzahl)</span> <span class="text-xs italic">— «…ga con **Form 7004** hasta el **15 de octubre**. **Sanción: 25.000 USD por formulario y añ…»</span> <strong>[NICHT VERIFIZIERT]</strong></li>
<li><span class="font-mono">21 %</span> <span class="opacity-70">(Kennzahl)</span> <span class="text-xs italic">— «…the-box election* a C-Corp (Form 8832): entonces tributa al 21 % federal y presenta un 112…»</span> <strong>[REVISIÓN MANUAL — vorgeschlagene Quelle: <a href="https://www.irs.gov" rel="nofollow noopener" target="_blank">www.irs.gov</a>]</strong></li>
<li><span class="font-mono">30%</span> <span class="opacity-70">(Kennzahl)</span> <span class="text-xs italic">— «…ite documentar tu estatus de no residente para no retenerte 30%. La W-8BEN-E vive 3 años o…»</span> <strong>[NICHT VERIFIZIERT]</strong></li>
<li><span class="font-mono">100%</span> <span class="opacity-70">(Kennzahl)</span> <span class="text-xs italic">— «…ers y emprendedores ya operan con su LLC americana de forma 100% legal y documentada. En E…»</span> <strong>[NICHT VERIFIZIERT]</strong></li>
<li><span class="font-mono">IRC §882</span> <span class="opacity-70">(Rechtsverweis)</span> <span class="text-xs italic">— «…§301.7701-3 (clasificación de entidades / *check-the-box*); IRC §882 (impuesto sobre renta…»</span> <strong>[REVISIÓN MANUAL — vorgeschlagene Quelle: <a href="https://www.irs.gov" rel="nofollow noopener" target="_blank">www.irs.gov</a>]</strong></li>
<li><span class="font-mono">IRC §871</span> <span class="opacity-70">(Rechtsverweis)</span> <span class="text-xs italic">— «…rentas de extranjeros conectadas con US trade or business); IRC §871 (FDAP y retenciones a…»</span> <strong>[REVISIÓN MANUAL — vorgeschlagene Quelle: <a href="https://www.irs.gov" rel="nofollow noopener" target="_blank">www.irs.gov</a>]</strong></li>
<li><span class="font-mono">IRC §6038</span> <span class="opacity-70">(Rechtsverweis)</span> <span class="text-xs italic">— «…r business); IRC §871 (FDAP y retenciones a no residentes); IRC §6038A y Treas. Reg. §1.60…»</span> <strong>[REVISIÓN MANUAL — vorgeschlagene Quelle: <a href="https://www.irs.gov" rel="nofollow noopener" target="_blank">www.irs.gov</a>]</strong></li>
<li><span class="font-mono">IRC §7701</span> <span class="opacity-70">(Rechtsverweis)</span> <span class="text-xs italic">— «…25% foreign-owned* y *foreign-owned disregarded entities*); IRC §7701(b) (residencia fisca…»</span> <strong>[REVISIÓN MANUAL — vorgeschlagene Quelle: <a href="https://www.irs.gov" rel="nofollow noopener" target="_blank">www.irs.gov</a>]</strong></li>
<li><span class="font-mono">Form 5472</span> <span class="opacity-70">(Rechtsverweis)</span> <span class="text-xs italic">— «…r el **&lt;a href=&quot;/es/blog/form-5472-que-es-como-presentarlo&quot;&gt;Form 5472&lt;/a&gt; + Form 1120** an…»</span> <strong>[REVISIÓN MANUAL — vorgeschlagene Quelle: <a href="https://www.irs.gov" rel="nofollow noopener" target="_blank">www.irs.gov</a>]</strong></li>
<li><span class="font-mono">Form 1120</span> <span class="opacity-70">(Rechtsverweis)</span> <span class="text-xs italic">— «…/es/blog/form-5472-que-es-como-presentarlo&quot;&gt;Form 5472&lt;/a&gt; + Form 1120** ante el IRS. Es un…»</span> <strong>[REVISIÓN MANUAL — vorgeschlagene Quelle: <a href="https://www.irs.gov" rel="nofollow noopener" target="_blank">www.irs.gov</a>]</strong></li>
<li><span class="font-mono">Form 7004</span> <span class="opacity-70">(Rechtsverweis)</span> <span class="text-xs italic">— «…esto que pagar). Es básicamente un sobre para el 5472. ### Form 7004, la extensión automát…»</span> <strong>[REVISIÓN MANUAL — vorgeschlagene Quelle: <a href="https://www.irs.gov" rel="nofollow noopener" target="_blank">www.irs.gov</a>]</strong></li>
<li><span class="font-mono">Form 114</span> <span class="opacity-70">(Rechtsverweis)</span> <span class="text-xs italic">— «…//www.fincen.gov&quot; target=&quot;_blank&quot; rel=&quot;noopener&quot;&gt;FinCEN&lt;/a&gt; Form 114) Si la LLC tiene cuen…»</span> <strong>[REVISIÓN MANUAL — vorgeschlagene Quelle: <a href="https://www.irs.gov" rel="nofollow noopener" target="_blank">www.irs.gov</a>]</strong></li>
<li><span class="font-mono">Form 8832</span> <span class="opacity-70">(Rechtsverweis)</span> <span class="text-xs italic">— «…a si la LLC ha realizado *check-the-box election* a C-Corp (Form 8832): entonces tributa a…»</span> <strong>[REVISIÓN MANUAL — vorgeschlagene Quelle: <a href="https://www.irs.gov" rel="nofollow noopener" target="_blank">www.irs.gov</a>]</strong></li>
<li><span class="font-mono">Form 8822</span> <span class="opacity-70">(Rechtsverweis)</span> <span class="text-xs italic">— «…l constituir). El EIN es de por vida; cambias dirección con Form 8822-B, no con el EIN. La…»</span> <strong>[REVISIÓN MANUAL — vorgeschlagene Quelle: <a href="https://www.irs.gov" rel="nofollow noopener" target="_blank">www.irs.gov</a>]</strong></li>
<li><span class="font-mono">RD 1065/2007</span> <span class="opacity-70">(Rechtsverweis)</span> <span class="text-xs italic">— «…cionador del Modelo 720 tras STJUE C-788/19 de 27/01/2022); RD 1065/2007 (Modelos 232 y 72…»</span> <strong>[REVISIÓN MANUAL — vorgeschlagene Quelle: <a href="https://www.boe.es" rel="nofollow noopener" target="_blank">www.boe.es</a>]</strong></li>
<li><span class="font-mono">DAC6</span> <span class="opacity-70">(Rechtsverweis)</span> <span class="text-xs italic">— «…oopener&quot;&gt;OCDE&lt;/a&gt;.** Directiva (UE) 2011/16, modificada por DAC6 (mecanismos transfronteri…»</span> <strong>[REVISIÓN MANUAL — vorgeschlagene Quelle: <a href="https://eur-lex.europa.eu" rel="nofollow noopener" target="_blank">eur-lex.europa.eu</a>]</strong></li>
<li><span class="font-mono">DAC7</span> <span class="opacity-70">(Rechtsverweis)</span> <span class="text-xs italic">— «…2011/16, modificada por DAC6 (mecanismos transfronterizos), DAC7 (Directive (EU) 2021/514,…»</span> <strong>[REVISIÓN MANUAL — vorgeschlagene Quelle: <a href="https://eur-lex.europa.eu" rel="nofollow noopener" target="_blank">eur-lex.europa.eu</a>]</strong></li>
<li><span class="font-mono">DAC8</span> <span class="opacity-70">(Rechtsverweis)</span> <span class="text-xs italic">— «…s), DAC7 (Directive (EU) 2021/514, plataformas digitales) y DAC8 (criptoactivos); Directiv…»</span> <strong>[REVISIÓN MANUAL — vorgeschlagene Quelle: <a href="https://eur-lex.europa.eu" rel="nofollow noopener" target="_blank">eur-lex.europa.eu</a>]</strong></li>
</ul>
</aside>
<!-- /exentax:review-anchor-v1 -->

`;
