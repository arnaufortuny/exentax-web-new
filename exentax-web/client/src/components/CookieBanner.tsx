  ...
  return (
    <button
      ...
      className={`relative w-9 h-5 rounded-full transition-colors duration-200 flex-shrink-0 ${
        checked ? "bg-[var(--green)]" : "bg-[var(--muted)]"
...
export default function CookieBanner() {
  ...
  useEffect(() => {
    if (import.meta.env.DEV && new URLSearchParams(window.location.search).get("_nb") === "1") {
      setVisible(false);
    } else if (getCookieConsent() === null) {
      setVisible(true);
    }
    ...
    fetch("/api/legal/versions", { signal: ac.signal }).then(r => { if (!r.ok) throw new Error(`HTTP ${r.status}`); return r.json(); }).then(data => {
      if (data?.versions?.cookies?.version) setCookieDocVersion(data.versions.cookies.version);
    }).catch((e) => {
      if (e?.name === "AbortError") return;
      if (e instanceof TypeError) return;
      clientLogger.warn("[cookie-banner] legal versions fetch failed", e);
    });
  ...
  return (
    <div className="fixed bottom-0 left-0 right-0 p-4 sm:p-5" style={{ pointerEvents: "none", zIndex: 70 }}>
      <div
        ...
        data-testid="dialog-cookie-banner"
        ...
        <div className="p-5 sm:p-6">
          <div className="flex items-start gap-3 mb-4">
            ...
            <div className="flex-1">
              <h4 id="cookie-banner-title" className="font-body font-semibold text-sm sm:text-base text-[var(--text-1)] mb-1.5">{t("cookie.title")}</h4>
              <p id="cookie-banner-desc" className="text-[var(--text-2)] text-sm leading-relaxed mb-2">{t("cookie.desc")}</p>
              <p className="text-[var(--text-2)] text-sm leading-relaxed">
                {t("cookie.desc2")}{" "}
                <Link href={lp("legal_cookies")} className="text-[var(--green)] hover:underline font-medium">
                  {t("cookie.policyLink")}
                </Link>.
              </p>
            </div>
          ...
          {showDetails && (
            <div className="mb-3 rounded-xl border border-[var(--border)] bg-[var(--bg-1)] p-3 space-y-2.5">
              <div className="flex items-center justify-between">
                ...
                <span className="text-[10px] font-semibold text-[var(--green)] bg-[rgba(0,229,16,0.10)] px-2 py-0.5 rounded-full">{t("cookie.alwaysActive")}</span>
              </div>
          ...
          <div className="flex flex-col sm:flex-row items-stretch sm:items-center gap-2.5 sm:gap-2">
            <button
              onClick={() => accept("all")}
              disabled={saving}
              className="btn-primary flex-1 font-body font-semibold text-sm sm:text-xs rounded-full px-6 py-3 sm:py-2.5 cursor-pointer disabled:opacity-60 disabled:cursor-not-allowed"
            ...
            <div className="flex items-center gap-2">
              ...
              {!showDetails ? (
                <button
                  ...
                  disabled={saving}
                  className="text-[var(--text-3)] hover:text-[var(--green)] font-body font-medium text-sm sm:text-xs rounded-full px-5 py-3 sm:py-2.5 transition-colors duration-200 cursor-pointer whitespace-nowrap disabled:opacity-60 disabled:cursor-not-allowed"
                ...
                <button
                  ...
                  disabled={saving}
                  className="text-[var(--green)] hover:text-[var(--green-hover)] font-body font-semibold text-sm sm:text-xs rounded-full px-5 py-3 sm:py-2.5 transition-colors duration-200 cursor-pointer whitespace-nowrap disabled:opacity-60 disabled:cursor-not-allowed"
