...
export default function Layout({ children }: { children: React.ReactNode }) {
  ...
  return (
    <div className="relative w-full overflow-x-hidden flex flex-col min-h-screen" style={{ backgroundColor: "var(--bg-0)" }}>
      <Tracking />
      <a href="#main-content" data-testid="link-skip-to-main" className="sr-only focus:not-sr-only focus:absolute focus:top-2 focus:left-2 focus:z-[9999] focus:px-4 focus:py-2 focus:bg-black focus:text-white focus:rounded-lg focus:text-sm">
        {t("common.skipToMain")}
      </a>
      <Navbar />
