...
### 3.4 Maintenance scripts (one-shot, idempotent)

Two utility scripts live alongside the validators and can be re-run on demand
when bulk content changes (e.g. a wave of new articles) drop the graph below
the §1.4 / §3.2 thresholds:

- `node scripts/seo-add-cross-refs.mjs` — for every Spanish article with fewer
  than 3 incoming links, picks the most topically related sources (by
  `category` + `keywords` Jaccard score from `client/src/data/blog-posts.ts`)
  and appends a marker-wrapped "Más lecturas relacionadas" block to those
  source articles' bodies. The `<!-- exentax:cross-refs-v1 -->` …
  `<!-- /exentax:cross-refs-v1 -->` markers make the script idempotent: a
  re-run rewrites the block in place without duplicating it.
- `node scripts/seo-fix-broken-links.mjs` — scans every per-article content
  file in all locales and reports `/<lang>/blog/<slug>` hrefs that don't
  resolve in `BLOG_SLUG_I18N`. For each broken href it tries to find the
  closest valid slug for the same locale (Jaccard over slug tokens) and
  rewrites the link in place when the score ≥ 0.5. Borderline cases are
  printed as "Ambiguous (no fix applied)" and require manual review against
  the canonical slug map; the script intentionally does not write on low
  confidence to avoid introducing semantically wrong links.

Both scripts are intentionally *not* part of `npm run check`: they mutate
content files and should run interactively (commit the diff, then re-run
`npm run seo:check` to confirm green).
