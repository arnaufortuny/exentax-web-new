...
- **Robots.txt** correctly disallows `/api/`, `/links`, `/start`, `/booking/` and declares the sitemap.
...
## 9. Implementation status (April 17, 2026 — task #5)

Concrete code changes shipped as part of the production-readiness audit:

- **Sitemap freshness fix** (§2.3): `lastmod` for main pages and the blog index now uses a content-based constant (`2026-04-01`) instead of `todayMadridISO()`, eliminating the freshness-spam signal. Bump these constants when copy actually changes (`server/routes/public.ts`).
- **Strong typing** (§2.5): replaced `allJsonLd: any[]` with `JsonLdSchema[]` (`Record<string, unknown>[]`) in `server/static.ts`. `tsc --noEmit` runs clean.
- **Production env fail-fast**: `server/index.ts` now refuses to start in production if any of `FIELD_ENCRYPTION_KEY`, `GOOGLE_SERVICE_ACCOUNT_KEY`, `DISCORD_BOT_TOKEN`, or `DISCORD_CHANNEL_REGISTROS` are missing — booking, PII encryption at rest, the Discord admin bot, and lead notifications all depend on these. `DATABASE_URL` is required in every environment.
- **Hreflang coverage verified**: all 65 published blog posts (count from `npm run seo:check`) have full `title` + `metaDescription` translations in en/fr/de/pt/ca, so the cross-language `<xhtml:link rel="alternate">` block emitted by `injectMeta` and the sitemap is reciprocal and complete.
- **Robots.txt**: confirmed coverage for `/api/`, `/links`, `/start`, `/booking/` (which already covers `/booking/manage/`) plus the UTM/ref/gclid/fbclid/mc_* attribution noise patterns.
- **Legacy unprefixed Spanish URLs**: 301 middleware in `server/index.ts` already redirects `/sobre-las-llc`, `/blog`, `/blog/*`, `/legal/*`, etc. to their `/es/...` canonical, eliminating duplicate-content risk.
