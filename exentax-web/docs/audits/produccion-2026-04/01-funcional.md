...
### Hallazgos adicionales del architect (verificados)

Un segundo arquitecto sugirió 6 vectores. Cada uno se verificó contra
el código real para distinguir bug de falsa alarma:

| # | Sugerencia del architect | Verificación contra el código | Disposición |
|---|--------------------------|-------------------------------|-------------|
| A1 | "Filas de `discord_outbound_queue` rehidratadas pero nunca enviadas" | `drainTick` (server/discord.ts:631–691) llama a `claimDueRows()` (línea 701) que hace `SELECT … FOR UPDATE SKIP LOCKED` sobre la tabla. Las filas DB sí se procesan. La rehidratación de `_pendingCount` en `loadBackend` (líneas 590–602) es solo para el gauge de métricas. | **Falso positivo**. No se cambia nada. |
| A2 | "Drip worker: si dispatch éxito pero `advance` falla y luego `markError` también falla → claim queda 10 min → email duplicado" | `markDripEnrollmentError` (storage/marketing.ts:246–252) libera el claim (`claimedAt: null`). El edge case de doble-fallo (advance + markError) sí existe; tras F2 (drip-worker.ts:210), ese doble fallo ya se loguea. La duplicación tras 10 min es un trade-off conocido y documentado en el header del worker. | **Mitigado** por F2 + comportamiento documentado. |
| A3 | "Rate limit Map evicta usuarios legítimos bajo IP-rotation attack" | El eviction LRU (server/index.ts:509–516) sí drop oldest, pero un usuario evictado solo ve su contador resetearse — NO queda bloqueado. El cap de 10K × ~50 B = 500 KB no es vector de RAM. Además solo se usa cuando `_globalRlStore` (Redis) no está disponible. | **No es bug real**. Documentado. |
| A4 | "Side effects post-tx en booking (scheduleReminder, notifyDiscord, logConsent) → si crashea entre líneas 700 y 720 quedan sin ejecutar" | Real: `scheduleReminderEmail` es timer en memoria + sweeper DB (recovery existe). `notifyBookingCreated` usa `discord_outbound_queue` persistente (recoverable). `logConsent` escribe directo a DB (atomic). El único que se perdería en crash entre las llamadas síncronas es la INTENCIÓN de encolar — pero las funciones son síncronas hasta el primer `await` interno, así que la ventana es de microsegundos. Mover todo a la transacción exigiría rediseñar emails/Discord/consent (job queue persistente). | **Documentado como deuda técnica** para tarea dedicada (`#28+`). Riesgo aceptado: ventana de microsegundos en path no-fatal. |
| A5 | "`checkCsrfOrigin` over-reliance on Origin" | Verificado en route-helpers.ts:418–431. Sí valida `origin`, cae a `referer` si origin es vacío o `"null"`, y rechaza si ambos faltan. `isTrustedOrigin` valida hostname contra lista cerrada (SITE_URL + replit dev domains). Implementación estándar y correcta. | **Falso positivo**. |
| A6 | "`tx as unknown as DbOrTx` permite que dev use `db` global por accidente" | server/db.ts:260 pasa `tx` como parámetro `fn(tx)`. El cast es solo para satisfacer TS. Para "usar db por accidente" un dev tendría que importar el `db` global Y usarlo dentro del callback explícitamente — no es accidente, es decisión de código. | **Falso positivo**. |
