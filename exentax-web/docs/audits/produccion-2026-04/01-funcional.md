...
|---|--------------------------|----------------------------------------|-------------|
| A1 | "Filas de `discord_outbound_queue` rehidratadas pero nunca enviadas" | Verificado: `drainTick` SÍ procesa filas DB vía `claimDueRows()` con `SELECT … FOR UPDATE SKIP LOCKED`, así que NO hay un bug de envíos no procesados. **Pero la métrica original sí estaba sesgada** bajo co-tenancy (rolling deploy, dev + scripts ó audit): (i) `loadBackend` inflaba `_pendingCount` con todo el backlog persistido aunque otro proceso fuese a drainearlo; (ii) `claimDueRows()` que reclamase una fila enqueueada por un peer disparaba un `decPending()` local, llevando nuestro gauge a 0 mientras filas propias seguían pendientes; (iii) recíprocamente, una fila propia drainada por un peer dejaba nuestro gauge inflado, y co-tenants publicaban valores DISTINTOS de `discord_queue_size`. **Fix estructural aplicado en T#36** unificando el gauge publicado en una sola fuente cross-proceso: (1) `loadBackend` ya NO toca `_pendingCount` — solo registra log informativo del backlog. (2) Nueva variable `_publishedQueueSize` actúa como gauge canónico cross-proceso publicado vía `getDiscordQueueSize()` y `discord_queue_size` (Prometheus + JSON). Se ajusta optimistamente en `incPending()`/`decPending()` (delta local instantáneo para tests single-process) y se RECONCILIA contra `SELECT count(*) FROM discord_outbound_queue WHERE attempts < max_attempts` en `refreshPublishedQueueDepth()`, llamado: (a) en cada `drainTick` (cada `DRAIN_INTERVAL_MS = 1.5 s`), (b) en `startDiscordQueueWorker()` al boot, y (c) sincrónicamente en cada scrape de `/api/metrics` antes de responder, con un `Promise.race` de 1 s para acotar latencia bajo DB degradada. (3) Tracking interno per-process retenido como diagnóstico no-publicado: `_pendingCount` + `_ownDbRowIds: Set<string>` (poblado en `persistAndQueue` tras INSERT) se exponen solo vía `_getDiscordOwnPendingCountForTests()` para asserts del test de persistencia. (4) Test `tests/discord-queue-persistence.test.ts` actualizado al nuevo contrato: tras `_resetDiscordQueueForTests()` + `startDiscordQueueWorker()` con una fila peer en DB, `getDiscordQueueSize() ≥ 1` (porque startup hace `refreshPublishedQueueDepth()`), mientras `_getDiscordOwnPendingCountForTests() === 0`. Helpers ad-hoc anteriores (`getDiscordQueueDepthFromDb()`, `getDiscordQueueDepthFromDbDetailed()`, `discord.queueDepthDb`) eliminados — colapsan en el gauge canónico unificado. **Contrato de convergencia honesto (no incondicional):** los co-tenants convergen al mismo valor dentro de ≤1.5 s **bajo dos asunciones operativas**: (a) cada proceso ejecuta su `drainTick` con regularidad — un proceso con event-loop estancado o sin worker arrancado puede divergir hasta que el siguiente tick se ejecute; (b) el `SELECT count(*)` succeed — bajo fallos repetidos de `refreshPublishedQueueDepth` (DB inalcanzable), el proceso afectado sirve el último valor cacheado y emite `WARN [discord]` por scrape (señal de degradación detectable por SRE), mientras peers con DB sana siguen publicando valores frescos. En `/api/metrics`, además, el scrape se desbloquea en ≤1 s incluso si la DB tarda (no se cuelga hasta `statement_timeout`). Validación: tsc clean, discord regression 6/6 (72/72 bot E2E), persistence test 8/8, drip-exactly-once 37/37, consent-atomicity 12/12, booking 54/54, newsletter 55/55. | **RESUELTO en T#36** — gauge único cross-proceso convergente ≤1.5 s con asunciones de healthy-refresh; degradación de DB observable vía WARN log + bounded scrape latency. |
| ~~A2~~ | "Drip worker: si dispatch éxito pero `advance` falla y luego `markError` también falla → claim queda 10 min → email duplicado" | **Historia**: T#36 cerró la race condition común con un sentinel `last_sent_step` + claim atómico para `sendImmediateStep1`, pero dejó tres residuales conocidos como deuda: (a) ventana entre el ACK SMTP y la primera escritura del sentinela; (b) lease overlap si la ruta inmediata se colgaba >10 min antes del envío SMTP; (c) doble fallo sentinel + poison tras SMTP exitoso. Estos residuales requerían un cambio estructural — implementado en T#38. **RESUELTO en T#38 (30-abr-2026)** mediante refactor estructural a **outbox transaccional con fencing token**. Nueva tabla `email_outbox` (`shared/schema.ts`, migración idempotente en `runColumnMigrations`) con FK a `drip_enrollments`, unique `(enrollment_id, step)`, índice parcial `WHERE sent_at IS NULL`, columnas `claim_version` (fencing token), `attempts`/`max_attempts`, `next_attempt_at`, `payload` JSON. La intención "este email debe enviarse" se persiste **dentro de la misma `withTransaction`** que crea `drip_enrollments` + `consent_log` + `newsletter_subscribers` (rutas `/api/newsletter/subscribe` y `/api/calculator/save` en `routes/public.ts`), eliminando el fire-and-forget post-commit. Un único `drainOutbox()` en `scheduled/drip-worker.ts` reclama filas con `claimDueOutboxRows` (incrementa `claim_version` + `attempts` atómicamente, `SELECT … FOR UPDATE SKIP LOCKED`), dispatcha SMTP (`sendOutboxRow`), y persiste `markOutboxRowSent` + `advanceDripEnrollment(tx)` + `lastSentStep` UPDATE + `enqueueOutboxRow(nextStep, tx)` **en una única transacción adicional** que valida la CAS sobre `claim_version` en cada UPDATE — TODOS los writes post-SMTP comparten la misma `tx` y comprometen atómicamente o ninguno. Mapeo 1-a-1 a los residuales eliminados: (a) la ventana ACK→sentinel se colapsa a una sola transacción; tras los reintentos en-tx, si la sentinel sigue fallando, `poisonOutboxRow` parquea la fila a `next_attempt_at = 9999-12-31` — el siguiente claim no la re-elige. (b) Una recamada con lease stale por otro worker bumpa el `claim_version`; el worker rezagado pasa por una **fence pre-SMTP** (`getOutboxClaimVersion(row.id)` en `sendOutboxRow`) que detecta el supersedimiento y abandona ANTES de dispatchear SMTP — eliminando el duplicado al nivel de entrega, no solo al nivel de DB. Aun en la ventana microscópica entre la fence y el SMTP, el UPDATE belated del worker original no-opea silenciosamente vía CAS sobre `markOutboxRowSent` — nunca hay doble escritura del sentinel. (c) `attempts` se incrementa en el claim (no en el dispatch), así que aun si `poisonOutboxRow` también falla, después de `max_attempts` reclamos la fila cae fuera del filtro de elegibilidad — el peor caso es un puñado fijo de SMTPs duplicados a lo largo de la vida de la fila, no un loop infinito. Helpers legacy `tryClaimDripEnrollmentForImmediate` / `sendImmediateStep1` quedan retirados de la ruta de routes; `markDripStepSent` se mantiene solo como UPDATE inline dentro de la tx post-send para sincronizar `last_sent_step` (observabilidad legacy). Endpoints: `wakeOutboxWorker()` post-commit dispara un drain inmediato (UX simétrica a la antigua `void sendImmediateStep1`, pero todo va por el worker único con fencing). Helper de migración `backfillOutboxFromEnrollments()` corre al boot del worker para enqueuear filas de outbox para enrollments legacy en vuelo (idempotente vía unique `(enrollment_id, step)`). Cubierto por `tests/drip-exactly-once.test.ts` reescrito (5 escenarios alineados con los residuales): (a) SMTP-OK + sentinel-fail → poisoned, sin duplicado entre passes; (b) lease overlap a nivel CAS → CAS rechaza el mark belated del worker A, worker B persiste exactamente uno; (b') lease overlap a nivel SMTP → fence pre-send detecta el supersedimiento del `claim_version` y abandona el worker A ANTES de dispatchear, worker B dispatcha → exactamente UN SMTP across both workers; (c) markSent + poison ambos fallan → SMTPs acotados a `MAX_ATTEMPTS`; (d) atomicidad del post-send: forzar fallo en `advanceDripEnrollment` dentro de la tx → ninguno de los writes (sentinel, advance, next-step enqueue) se compromete, el row vuelve a estar reclamable en estado consistente. | **RESUELTO en T#38** — outbox transaccional con fencing token elimina los tres residuales (a)/(b)/(c) bajo failure modes adversariales. Garantía: el set de writes post-SMTP compromete-todos-o-ninguno; el `claim_version` actúa como fencing token estándar (Lamport / Helland) que invalida cualquier writer rezagado. Test de regresión cubre los tres residuales más el caso de partial-commit, todos pass. |
