...
|---|--------------------------|----------------------------------------|-------------|
| A1 | "Filas de `discord_outbound_queue` rehidratadas pero nunca enviadas" | Verificado: `drainTick` SÍ procesa filas DB vía `claimDueRows()` con `SELECT … FOR UPDATE SKIP LOCKED`, así que NO hay un bug de envíos no procesados. **Pero la métrica original sí estaba sesgada** bajo co-tenancy (rolling deploy, dev + scripts ó audit): (i) `_pendingCount += recovered` en `loadBackend` inflaba el gauge de cada proceso con todo el backlog persistido aunque otro proceso fuese a drainearlo, y (ii) más sutil — incluso sin la rehidratación, una `claimDueRows()` que reclamase una fila enqueueada por otro proceso disparaba un `decPending()` aquí, llevando nuestro gauge a 0 mientras filas propias seguían pendientes; recíprocamente, una fila propia drainada por un peer dejaba nuestro gauge inflado para siempre. **Fix estructural aplicado en T#36** combinando ambas opciones del task spec: (1) `loadBackend` ya NO toca `_pendingCount` — solo registra log informativo del backlog en disco. (2) Se añade tracking explícito de propiedad por proceso: `_ownDbRowIds: Set<string>` se popula en `persistAndQueue` tras un INSERT exitoso (`trackOwnedDbRow(row.id)`); `decPending(persistedId)` solo decrementa si el id está en el set (filas peer = no-op silencioso). Items de la cola en memoria (`persistedId === null`) son siempre nuestros por construcción y decrementan siempre. (3) Drift residual documentado **honestamente**: si un peer drainea una fila que nosotros enqueueamos, nuestro `_pendingCount` queda con +1 hasta el siguiente reset/restart del proceso. Esto **NO es acotado** en términos operativos — bajo throughput sostenido y co-tenancy continua, el gauge per-process puede acumular drift indefinidamente (igual que `_ownDbRowIds`). Sin impacto en correctitud de envío (las filas peer se procesan correctamente; solo el gauge local diverge). Para observabilidad cross-proceso fiable usar siempre `getDiscordQueueDepthFromDb()` / `discord.queueDepthDb`. Reconciliación verdadera del gauge per-process requeriría una columna `producer_instance_id` en `discord_outbound_queue` y purga periódica de `_ownDbRowIds` contra la DB — registrado como follow-up #39 (no-bloqueante: el gauge per-process es informativo, no de toma de decisiones). (4) Para SRE se añade helper async `getDiscordQueueDepthFromDb()` (DB COUNT con caché de 5 s) expuesto como `discord.queueDepthDb` en `/api/metrics` (separado del gauge per-process `discord.queueSize`). El test `tests/discord-queue-persistence.test.ts` se migró: filas insertadas directamente por el test (= peer) verifican `getDiscordQueueDepthFromDb() ≥ 1` y `getDiscordQueueSize() === 0`, alineado con el contrato. Comentario "by definition disjoint" eliminado. Validación: tsc clean, discord regression 6/6 (72/72 bot E2E), persistence test 8/8, drip-exactly-once 37/37, consent-atomicity 12/12, booking 54/54, newsletter 55/55. | **RESUELTO en T#36** — gauge per-process honesto con tracking de propiedad explícito; DB COUNT cross-proceso para SRE. |
