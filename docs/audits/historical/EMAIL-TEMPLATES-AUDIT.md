...
- **Backward-compatible** — `upsertNewsletterSubscriber()` still returns the row shape the existing callers expect (`unsubscribeToken`, `email`, etc.); the `isNew` field is purely additive. Verified against the e2e test script (`scripts/e2e/test-newsletter-e2e.ts`) and the calculator-subscribe call site.
