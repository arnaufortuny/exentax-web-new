...
## Comprehensive Hardening (April 2026)
- **Error handling**: All silent `.catch(() => {})` blocks in rate-limit store initialization and cleanup now log warnings with proper `logger.warn(message, source)` signature. 4 locations fixed across `index.ts` and `route-helpers.ts`.
- **Phone validation alignment**: Frontend booking and calculator forms now validate 7-15 digits, matching backend `isValidPhone()` (was only checking >=7). PhoneInput component has `maxLength={20}` attribute. All 6 locale `phoneError` translations updated to reflect "7 to 15 digits".
- **Full audit results**: TypeScript 0 errors. i18n 0 missing/extra/empty keys. No dead components, hooks, or imports. No TODO/FIXME/HACK comments. No unused exports. All routes active and consistent. Schema matches all form fields, API contracts, and Discord embeds. Circuit breaker pattern solid. Admin auth uses timing-safe comparison. Slot locking with 15s timeout. All consent timestamps use ISO format (UTC) for audit trail.
- **Architecture verified**: Storage layer (core/scheduling/marketing/legal) properly separated. Route validation uses Zod `.strict()` schemas. Global error handler maps error types to HTTP codes. Fire-and-forget patterns (email, Discord, Sheets) have proper `.catch()` logging. CSRF origin check on all non-GET `/api/` routes.

## Critical Resilience Hardening (April 2026)
- **Race condition eliminated**: Reschedule operations now use dual lock (`withBookingLock` → `withSlotLock`) to prevent cancel+reschedule races from overwriting each other. Status re-verified inside lock with fresh DB read. Both public and admin routes protected.
- **Booking lock system**: New `withBookingLock(bookingId, fn)` serializes all state-changing operations (reschedule, cancel) on the same booking. Cancel operations wrapped in booking lock with fresh status check to prevent duplicate cancellations and duplicate email/webhook side effects.
- **Same-slot guard**: Reschedule to current slot returns `SAME_SLOT` error instead of false `SLOT_TAKEN`. Added `sameSlot` backend label in all 6 languages.
- **Calculator error feedback**: Lead submission failures now show user-facing error message instead of failing silently. New `calculator.sendError` i18n key in all 6 locales.
- **Lock ordering**: Consistent lock acquisition order (bookingLock → slotLock) prevents deadlocks. In-memory implementation suitable for single-instance; code comments document Redis upgrade path for multi-instance.
- **State transition summary**: Booking transitions validated: pending→rescheduled→cancelled are the only forward paths. No resurrection of cancelled bookings possible. No-show is a terminal admin state. All transitions re-check fresh state inside locks.
